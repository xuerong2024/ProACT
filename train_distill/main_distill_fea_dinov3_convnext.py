...
def train_one_epoch(config, model, criterion, data_loader, optimizer, epoch, mixup_fn, lr_scheduler, loss_scaler, model_ema=None, writer=None):
    ...
    for idx, (images, masks,images2, masks2, targets, _) in enumerate(data_loader):
        ...
        with torch.no_grad():
            outputs_teacher,_,_,cls_token_teacher = model.teacher(samples2)
        cls_token_teacher=model.student.convnext_adapter_2(model.student.gelu(model.student.convnext_adapter_1(cls_token_teacher)))
        ...
        for qq in range(len(aug_gaussian)):
            for pp in range(len(aug_rot)):
                for jj in range(len(model_name)):
                    ...
                    config.OUTPUT = os.path.join(args.output, 'wmask_nosubcls_nodropout2','model_distill_fea','dinov3stu512_notrained_convnexttea',
