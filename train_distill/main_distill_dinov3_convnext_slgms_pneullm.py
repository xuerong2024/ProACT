...
def build_phe_loader(args):
    ...
    dataset_train = Shanxi_wmask_Dataset4(args.data_root + 'seg_rec_img_1024', args.data_root + 'seg_rec_mask_1024',args.data_root + 'seg_rec_mask_img_1024',
                                       txtpath=args.data_root + 'stage1_health_txt/selected_dcm_saomiao_900_health308_sick_592/train.txt',
                                       data_transform=global_transfo2, out_transform1=transform, out_transform2=transform_list2, out_transform3=transform_list3)
...
def main(config, ptname='model_ema_best_auc_shanxi_val'):
    ...
    model = MyModel(distiller_dict['model_slgms_teacher'], distiller_dict['model_convnext_teacher'], distiller_dict['model_pneum_teacher'], distiller_dict['model_dinov3_student'])
        ...
        for qq in range(len(kl_weights)):
            for pp in range(len(aug_rot)):
                for jj in range(len(model_name)):
                    ...
                    config.OUTPUT = os.path.join(args.output, 'wmask_nosubcls_nodropout','3models_distill',f'dinov3st224_notrained_convnext_slgms_pneullmtea_{args.kl_weight}weight',
