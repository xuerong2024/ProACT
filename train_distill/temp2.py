...
def parse_option():
    ...
    parser.add_argument('--output',
                        default='/disk3/wjr/workspace/sec_proj4/baseline_expeript/selected_dcm_saomiao_900_health308_sick_592/224/contra_20250906/',
...
class GaussianBlur(object):
    ...
    def __call__(self, img1, mask2, img2):
        do_it = random.random() <= self.prob
        if not do_it:
            return img1, mask2, img2
        return img1.filter(
...
import d_transform
import data.transforms as pth_transform
...
def build_phe_loader(args):
    ...
    transform_list = [
        d_transform.RandomResizedCrop(512, scale=(0.75, 1.), interpolation=Image.BICUBIC),
        d_transform.RandomHorizontalFlip(p=0.5)
    ]
    if args.aug_rot == True:
        transform_list.append(d_transform.RandomRotation(degrees=(-15, 15)))
    ...
    transform_list2 = pth_transform.Compose([
        pth_transform.ToTensor(),
        pth_transform.Normalize((0.485, 0.456, 0.406), (0.229, 0.224, 0.225)),
        # transforms.Normalize((0.48145466, 0.4578275, 0.40821073), (0.26862954, 0.26130258, 0.27577711)),
    ])
    transform_list3 = pth_transform.Compose([
        pth_transform.Resize((336, 336)),
        pth_transform.ToTensor(),
        pth_transform.Normalize((0.485, 0.456, 0.406), (0.229, 0.224, 0.225)),
        # transforms.Normalize((0.48145466, 0.4578275, 0.40821073), (0.26862954, 0.26130258, 0.27577711)),
    ])
    global_transfo2 = d_transform.Compose(transform_list)
    ...
    dataset_train = Shanxi_wmask_Dataset4(args.data_root + 'seg_rec_img_1024', args.data_root + 'seg_rec_mask_1024',
                                          args.data_root + 'seg_rec_mask_img_1024',
                                          txtpath=args.data_root + 'stage1_health_txt/selected_dcm_saomiao_900_health308_sick_592/train.txt',
                                          data_transform=global_transfo2, out_transform1=transform,
                                          out_transform2=transform_list2, out_transform3=transform_list3)

    dataset_train2 = Shanxi_Dataset(args.data_root + 'seg_rec_mask_img_1024',
                                    txtpath=args.data_root + 'stage1_health_txt/selected_dcm_saomiao_900_health308_sick_592/train.txt',
                                    data_transform=transform)

    dataset_val = Shanxi_Dataset(args.data_root + 'seg_rec_mask_img_1024',
    ...
    dataset_test = Shanxi_Dataset(args.data_root + 'seg_rec_mask_img_1024',
                                  ...
                                  data_transform=transform)
    dataset_test_dcm = Shanxi_Dataset(args.data_root + 'seg_rec_mask_img_1024',
                                      txtpath=args.data_root + 'stage1_health_txt/selected_dcm_saomiao_900_health308_sick_592/test_dcm.txt',
                                      data_transform=transform)

    dataset_test2 = Shanxi_Dataset('/disk3/wjr/dataset/nejm/guizhoudataset/seg_rec_mask_img_1024',
...
def main(config, ptname='model_ema_best_auc_shanxi_val'):
    data_loader_train, data_loader_train2, data_loader_val, data_loader_test, data_loader_test_dcm, data_loader_test2 = build_phe_loader(
        args)
    ...
    #                                                   mask_k_bias=True,
    #                                                   )
    distiller_dict['model_qwen_student'] = qwen2_5vision(n_class=2)
    ...
    distiller_dict['model_pneum_teacher'] = phellm()

    class MyModel(nn.Module):
        def __init__(self, teacher_model1, teacher_model2, teacher_model3, student_model):
    ...
    model = MyModel(distiller_dict['model_slgms_teacher'], distiller_dict['model_convnext_teacher'],
                    distiller_dict['model_pneum_teacher'], distiller_dict['model_qwen_student'])
    ...
    teacher_ckp_path3 = '/disk3/wjr/workspace/sec_nejm/nejm_baseline_expeript_new/allshanxi_sick_health/writer_analyze/512/allstage1_health1_1/selected_dcm_saomiao_900_health308_sick_592/pneumollm_336/87_83_llm_lr0.0008_weightsseed_10/trainable_bestvalpt.pth'
    checkpoint = torch.load(teacher_ckp_path3, map_location="cpu", weights_only=False)['model']
    model_dict = model.teacher3.state_dict()
    # 关键在于下面这句，从model_dict中读取key、value时，用if筛选掉不需要的网络层
    pretrained_dict = {key: value for key, value in checkpoint.items() if
                       (key in model_dict)}
    # 将参数加载到模型中
    msg_teacher3 = model.teacher3.load_state_dict(pretrained_dict, strict=False)
    # print(msg_teacher3, teacher_ckp_path3)
    model.to(device)
    # model_ema = ModelEma(
    #     model,
    ...
    for epoch in range(config.TRAIN.START_EPOCH, 20):
        ...
        writer = train_one_epoch(config, model, criterion, data_loader_train, optimizer, epoch, mixup_fn=None,
                                 lr_scheduler=lr_scheduler, loss_scaler=loss_scaler, writer=writer)
        # 保存当前RNG状态
        ...
        test_loss, test_auc, test_acc, writer = validate(data_loader_test, model.student, epoch, writer,
                                                         index='shanxi_test')
        ...
        loss_test2, auc_test2, test_acc2, writer = validate(data_loader_test2, model.student, epoch, writer,
                                                            index='guizhou_test')
...
def train_one_epoch(config, model, criterion, data_loader, optimizer, epoch, mixup_fn, lr_scheduler, loss_scaler,
    ...
    for idx, (images, images2, images3, targets, _) in enumerate(data_loader):
        # rand_module.randomize()
        samples = images3.to(device)
        samples2 = images.to(device)
        samples3 = images2.to(device)
        ...
        with torch.no_grad():
            outputs_teacher, _, _, _, _ = model.teacher2(samples2)
            outputs_teacher1 = model.teacher1(samples2)
            outputs_teacher3 = model.teacher3(samples3)
        ...
        pred_teacher3 = F.softmax(outputs_teacher3, dim=1)
        ...
        loss_kd3 = F.kl_div(log_pred_student, pred_teacher3, reduction="none").sum(1).mean()
        loss = loss_cls + args.kl_weight * (loss_kd1 + loss_kd2 + loss_kd3)
        optimizer.zero_grad()
        ...
        if idx % config.PRINT_FREQ == 0:
            ...
            writer.add_scalar('Shanxitrain pneullmtea_kd loss', loss_kd3.item(), epoch * num_steps + idx)
...
if __name__ == '__main__':
    ...
    # lrs = [5e-5]
    lrs = [3e-5, 1e-5, 5e-5, 8e-5, 1e-4, 3e-4]
    kl_weights = [0.5, 0.3, 0.8, 0.1]
    batch_sizes = [6]
    ...
    aug_gaussian = [True]
    loss_type = ['ce']
    pt_names = ['model_best_acc_alltest', 'model_best_acc_shanxi_val', 'model_best_auc_shanxi_val',
                'model_best_acc_shanxi_test', 'model_best_acc_guizhou_test']
    for kk in range(len(lrs)):
        for qq in range(len(kl_weights)):
            for pp in range(len(aug_rot)):
                for jj in range(len(model_name)):
                    ...
                    args.kl_weight = kl_weights[qq]
                    args.loss_type = loss_type[0]
                    args.aug_rot = True
                    args.aug_gaussian = True
                    config = get_config(args)
                    config.TRAIN.EPOCHS = 50
                    config.OUTPUT = os.path.join(args.output, 'wmask_nosubcls_nodropout', '3models_distill',
                                                 f'qwen3b_stu224_notrained_convnext_slgms_pneullmtea_{args.kl_weight}weight',
                                                 '224_batch6_lr' + str(args.base_lr) + '_testall')
