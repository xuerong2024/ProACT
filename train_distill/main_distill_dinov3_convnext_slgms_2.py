...
def build_phe_loader(args):
    ...
    dataset_train = Shanxi_wmask_Dataset3(args.data_root + 'seg_rec_img_1024',args.data_root + 'seg_rec_mask_1024',args.data_root + 'seg_rec_mask_img_1024',
...
def main(config, ptname='model_ema_best_auc_shanxi_val'):
    ...
    # pretrained_student = '/disk3/wjr/workspace/sec_proj4/baseline_expeript/selected_dcm_saomiao_900_health308_sick_592/224/contra_20250906/wmask_nosubcls_nodropout2/dinov3/vit-b/batch16_lr1e-05_testall/model_best_acc_guizhou_test.pth'
...
def train_one_epoch(config, model, criterion, data_loader, optimizer, epoch, mixup_fn, lr_scheduler, loss_scaler, model_ema=None, writer=None):
    ...
    for idx, (images, images2, targets, _) in enumerate(data_loader):
        ...
        loss = loss_cls+args.kl_weight*(loss_kd1+loss_kd2)
        optimizer.zero_grad()
        ...
        for qq in range(len(kl_weights)):
            for pp in range(len(aug_rot)):
                for jj in range(len(model_name)):
                    ...
                    config.OUTPUT = os.path.join(args.output, 'wmask_nosubcls_nodropout',f'2models_distill',f'dinov3stu224_notrained_convnext_slgmstea_{args.kl_weight}weight',
