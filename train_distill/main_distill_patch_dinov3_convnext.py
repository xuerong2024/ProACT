...
def main(config, ptname='model_ema_best_auc_shanxi_val'):
    ...
    model = MyModel(distiller_dict['model_convnext_teacher'], distiller_dict['model_dinov3_student'])
...
def train_one_epoch(config, model, criterion, data_loader, optimizer, epoch, mixup_fn, lr_scheduler, loss_scaler, model_ema=None, writer=None):
    ...
    for idx, (images, masks,images2, masks2, targets, imgnames) in enumerate(data_loader):
        ...
        with torch.no_grad():
            outputs_teacher,_,_,patch_token_teacher = model.teacher(samples2)
        ...
        loss_kd = fuc_losskd(patch_token, patch_token_teacher).mean(dim=2)
        ...
        loss_kd=(loss_kd*token_weight).sum(dim=1).mean()
        ...
        if idx % config.PRINT_FREQ == 0:
            ...
            writer.add_scalar('Shanxitrain kd loss', loss_kd.item(), epoch * num_steps + idx)
        ...
        for qq in range(len(aug_gaussian)):
            for pp in range(len(aug_rot)):
                for jj in range(len(model_name)):
                    ...
                    config.OUTPUT = os.path.join(args.output, 'wmask_nosubcls_nodropout2','model_distill_patch','dinov3stu224_notrained_convnexttea_stutokenweight',
