...
def build_phe_loader(args):
    ...
    dataset_train = Shanxi_wmask_Dataset3(args.data_root + 'seg_rec_img_1024', args.data_root + 'seg_rec_mask_1024',
                                       txtpath=args.data_root + 'stage1_health_txt/selected_dcm_saomiao_900_health308_sick_592/train.txt',
                                       data_transform=global_transfo2, out_transform1=transform, out_transform2=transform_list2, out_transform3=transform3)
...
def train_one_epoch(config, model, criterion, data_loader, optimizer, epoch, mixup_fn, lr_scheduler, loss_scaler, model_ema=None, writer=None):
    ...
    for idx, (images, masks,images2, masks2, images3, masks3,targets, _) in enumerate(data_loader):
        # rand_module.randomize()
        samples = (images * masks).to(device)
        samples2 = (images2 * masks2).to(device)
        samples3 = (images3 * masks3).to(device)
        ...
        loss = loss_cls+0.5*(loss_kd1+loss_kd2+loss_kd3)
        optimizer.zero_grad()
        ...
        for qq in range(len(aug_gaussian)):
            for pp in range(len(aug_rot)):
                for jj in range(len(model_name)):
                    ...
                    config.OUTPUT = os.path.join(args.output, 'wmask_nosubcls_nodropout2','3models_distill','dinov3stu224_notrained_convnext_slgms_pneullmtea',
