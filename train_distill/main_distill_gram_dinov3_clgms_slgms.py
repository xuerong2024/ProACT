...
def main(config, ptname='model_ema_best_auc_shanxi_val'):
    ...
    class MyModel(nn.Module):
        def __init__(self, teacher_model, teacher_model2, student_model):
    ...
    model = MyModel(distiller_dict['model_convnext_teacher'], distiller_dict['model_slgms_teacher'], distiller_dict['model_dinov3_student'])
    ...
    model.teacher.head.fc = nn.Linear(768, 2)
    ...
    msg_teacher = model.teacher.load_state_dict(checkpoint['model'], strict=True)
    print(msg_teacher, teacher_ckp_path)

    teacher_ckp_path2 = '/disk3/wjr/workspace/sec_nejm/nejm_baseline_expeript_new/allshanxi_sick_health/writer_analyze/512/allstage1_health1_1/selected_dcm_saomiao_900_health308_sick_592/slgms/9mixedsubs_2/s-vmamba_5e-5/checkpoint_best.pth.tar'
    state_dict = torch.load(teacher_ckp_path2, map_location="cpu", weights_only=False)['teacher']
    state_dict = {k.replace("backbone.", ''): v for k, v in state_dict.items()}
    msg_teacher2 = model.teacher2.load_state_dict(state_dict, strict=False)
    print(msg_teacher2, teacher_ckp_path2)

    # model.student.convnext_adapter_1 = nn.Linear(768, 1024)
    # model.student.convnext_adapter_2 = nn.Linear(1024, 768)
    # model.student.gelu = torch.nn.GELU()
    # model.student.slgms_adapter_1 = nn.Linear(768, 1024)
    # model.student.slgms_adapter_2 = nn.Linear(1024, 768)
    model.to(device)
    model_ema = ModelEma(
        model,
...
def train_one_epoch(config, model, criterion, data_loader, optimizer, epoch, mixup_fn, lr_scheduler, loss_scaler, model_ema=None, writer=None):
    ...
    fuc_losskd = nn.MSELoss(reduction='none')
    for idx, (images, masks,images2, masks2, targets, imgnames) in enumerate(data_loader):
        ...
        b,hw,c=patch_token.shape
        with torch.no_grad():
            outputs_teacher,_,_,cls_token_teacher,patch_token_teacher = model.teacher(samples2)
            outputs_teacher2, patch_token_teacher2, cls_token_teacher2 = model.teacher2(samples2, distill_token=True)

        # patch_token_teacher=model.student.convnext_adapter_2(model.student.gelu(model.student.convnext_adapter_1(patch_token_teacher)))
        # patch_token_teacher2 = model.student.slgms_adapter_2(
        #     model.student.gelu(model.student.slgms_adapter_1(patch_token_teacher2)))
        #
        # # 方法：转置 -> 插值 -> 转置回来
        # # 将 [b, 256, 1024] 转为 [b, 1024, 256]，在长度维度上插值
        b_t, hw_t,c_t=patch_token_teacher.shape
        if hw<hw_t:
            patch_token_teacher = F.interpolate(
                patch_token_teacher.transpose(1, 2),  # -> [b, 1024, 256]
                size=hw,  # 插值到长度 196
                mode='linear',  # 1D 线性插值
                align_corners=False,
                scale_factor=None
            ).transpose(1, 2)  # 输出 [b, 1024, 196]
            patch_token_teacher2 = F.interpolate(
                patch_token_teacher2.transpose(1, 2),  # -> [b, 1024, 256]
                size=hw,  # 插值到长度 196
                mode='linear',  # 1D 线性插值
                align_corners=False,
                scale_factor=None
            ...
            patch_token = F.interpolate(
                patch_token.transpose(1, 2),  # -> [b, 1024, 256]
                size=hw_t,  # 插值到长度 196
                mode='linear',  # 1D 线性插值
                align_corners=False,
                scale_factor=None
        ...
        patch_token_teacher = F.normalize(patch_token_teacher, dim=1)
        patch_token_teacher2 = F.normalize(patch_token_teacher2, dim=1)
        stu_gram= patch_token @ patch_token.transpose(-2, -1)
        tea_gram=patch_token_teacher @ patch_token_teacher.transpose(-2, -1)
        tea2_gram=patch_token_teacher2 @ patch_token_teacher2.transpose(-2, -1)
        stu_gram[stu_gram < 0] = 0.0
        tea_gram[tea_gram < 0] = 0.0
        tea2_gram[tea2_gram < 0] = 0.0

        loss_kd1=torch.nn.MSELoss()(stu_gram, tea_gram)
        loss_kd2 = torch.nn.MSELoss()(stu_gram, tea2_gram)
        loss_cls = criterion(outputs, targets)
        # log_pred_student = F.log_softmax(cls_token, dim=1)
        # pred_teacher = F.softmax(cls_token_teacher, dim=1)
        # loss_kd1 = F.kl_div(log_pred_student, pred_teacher, reduction="none").sum(1).mean()
        # pred_teacher2 = F.softmax(cls_token_teacher2, dim=1)
        # loss_kd2 = F.kl_div(log_pred_student, pred_teacher2, reduction="none").sum(1).mean()
        # loss_kd1 = fuc_losskd(patch_token, patch_token_teacher).mean(dim=2)
        # loss_kd2 = fuc_losskd(patch_token, patch_token_teacher2).mean(dim=2)

        # m_v_t = cls_token[:, None, :] @ patch_token.transpose(1, 2)  # [b, 1, 1024]
        # token_weight=m_v_t.squeeze(1)  # [b]
        # token_weight = torch.softmax(token_weight, dim=-1)
        #
        # loss_kd1=(loss_kd1*token_weight).sum(dim=1).mean()
        # loss_kd2 = (loss_kd2 * token_weight).sum(dim=1).mean()
        # m_v_t2 = cls_token[:, None, :] @ patch_token_teacher.transpose(1, 2)  # [b, 1, 1024]
        # token_weight2 = m_v_t2.squeeze(1)  # [b]
        # token_weight2 = torch.softmax(token_weight2, dim=-1)
        # loss_kd = F.kl_div(log_pred_student, pred_teacher, reduction="none").sum(1).mean()
        loss = loss_cls+0.5*loss_kd1+0.5*loss_kd2
        optimizer.zero_grad()
        ...
        if idx % config.PRINT_FREQ == 0:
            ...
            writer.add_scalar('Shanxitrain clgms kd loss', loss_kd1.item(), epoch * num_steps + idx)
            writer.add_scalar('Shanxitrain slgms kd loss', loss_kd2.item(), epoch * num_steps + idx)
            # writer.add_scalar('Shanxitrain cam_contra_sick_health_loss', loss_contraii.item(), epoch * num_steps + idx)
    # token_weight_img = token_weight.view(token_weight.shape[0], int(token_weight.shape[-1] ** 0.5),
    #                                          int(token_weight.shape[-1] ** 0.5))
    # token_weight_img = token_weight_img.detach().cpu().numpy()
    # token_weight_img2 = token_weight2.view(token_weight2.shape[0], int(token_weight2.shape[-1] ** 0.5),
    #                                      int(token_weight2.shape[-1] ** 0.5))
    # token_weight_img2 = token_weight_img2.detach().cpu().numpy()
    # savetoken_weight_path = os.path.join(config.OUTPUT, 'patch_token_sim')
    # Path(savetoken_weight_path).mkdir(parents=True, exist_ok=True)
    # plt.figure()
    # plt.imshow(token_weight_img[3, ...], cmap='hot', interpolation='nearest')  # 或 'bwr', 'viridis', 'jet' 等
    # plt.colorbar()  # 显示颜色条
    # plt.title("Feature Map Heatmap")
    # plt.axis('off')  # 关闭坐标轴
    # plt.tight_layout()
    # plt.show()
    # plt.savefig(savetoken_weight_path + f'/{imgnames[3]}stu_patch_sim_epoch{epoch}.png')
    # plt.close()  # 释放内存
    # plt.figure()
    # plt.imshow(token_weight_img[4, ...], cmap='hot', interpolation='nearest')  # 或 'bwr', 'viridis', 'jet' 等
    # plt.colorbar()  # 显示颜色条
    # plt.title("Feature Map Heatmap")
    # plt.axis('off')  # 关闭坐标轴
    # plt.tight_layout()
    # plt.show()
    # plt.savefig(savetoken_weight_path + f'/{imgnames[4]}stu_patch_sim_epoch{epoch}.png')
    # plt.close()  # 释放内存
    #
    # plt.figure()
    # plt.imshow(token_weight_img2[3, ...], cmap='hot', interpolation='nearest')  # 或 'bwr', 'viridis', 'jet' 等
    # plt.colorbar()  # 显示颜色条
    # plt.title("Feature Map Heatmap")
    # plt.axis('off')  # 关闭坐标轴
    # plt.tight_layout()
    # plt.show()
    # plt.savefig(savetoken_weight_path + f'/{imgnames[3]}tea_patch_sim_epoch{epoch}.png')
    # plt.close()  # 释放内存
    # plt.figure()
    # plt.imshow(token_weight_img2[4, ...], cmap='hot', interpolation='nearest')  # 或 'bwr', 'viridis', 'jet' 等
    # plt.colorbar()  # 显示颜色条
    # plt.title("Feature Map Heatmap")
    # plt.axis('off')  # 关闭坐标轴
    # plt.tight_layout()
    # plt.show()
    # plt.savefig(savetoken_weight_path + f'/{imgnames[4]}tea_patch_sim_epoch{epoch}.png')
    # plt.close()  # 释放内存
...
if __name__ == '__main__':
    ...
    aug_gaussian=[True]
    loss_type=['ce']
    pt_names=['model_best_acc_alltest','model_ema_best_acc_alltest','model_best_auc_shanxi_val','model_best_acc_shanxi_test','model_best_acc_guizhou_test']
        ...
        for qq in range(len(aug_gaussian)):
            for pp in range(len(aug_rot)):
                for jj in range(len(model_name)):
                    ...
                    config.OUTPUT = os.path.join(args.output, 'wmask_nosubcls_nodropout2','model_distill_gram','dinov3stu512_notrained_clgms_slgms_2teas_gramfeifutoken',
