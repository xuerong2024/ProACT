'''ESFDA: Energy-based Source-Free domain adaptation method for rotating machinery fault diagnosis, 2026 '''
import os
import time
import json
import random
...
def main(config, pt_path, logger_path):
    ...
    if args.model_name == 'resnet50':
        ...
        model = get_model('vit-base-p16_in21k-pre_3rdparty_in1k-384px', pretrained=pretrained_cfg,
                          backbone=dict(drop_path_rate=0.))
        ...
        model = get_model('vit-large-p16_in21k-pre_3rdparty_in1k-384px', pretrained=pretrained_cfg,
                          backbone=dict(drop_path_rate=0.))
    ...
    if os.path.isfile(ckp_path):
        ...
        if args.model_name == "slgms":
            ...
            msg = model.load_state_dict(state_dict, strict=False)
        model.to(device)
        shanxi_test_org = validate_network(data_loader_test, model, 'shanxi_test_org',
                                           save_path=logger_path + '/analyze/')
        guizhou_test_org = validate_network(data_loader_test2, model, 'guizhou_test_org',
                                            save_path=logger_path + '/analyze/')
...
class HSSCLoss(nn.Module):
    def __init__(self, num_classes=2, tau=1.0, epsilon=0.10):
...
class EnergyModel(nn.Module):
    ...
    def forward(self, x, y=None):
        ...
        if y is None:
            ...
            # return "wrong"
            # print('能量：', k)
            return logits, k  # 返回计算得到的能量以及logits

        else:
            return logits, torch.gather(logits, 1, y[:, None])
...
def adaptive_calculate_metrics(all_outputs, all_targets_onehot, threshold=None, valdata='shanxi_test', num_classes=2,
                               plot_roc=True, save_fpr_tpr=True, save_path=''):
...
def train_one_epoch(config, model, data_loader, buffer_size=10000, sgld_steps=10, sgld_lr=1, sgld_std=0.01,
                    reinit_freq=0.05, if_cond=False, ):
    energy_model = EnergyModel(model)

    log_path = os.path.join(config.OUTPUT, 'logs')
    os.makedirs(os.path.dirname(log_path), exist_ok=True)
    optimizer = build_optimizer(config, energy_model)
    ...
    for name, module in energy_model.named_modules():
    ...
    for epoch in range(config.TRAIN.EPOCHS):
        ...
        with torch.no_grad():
            ...
            for idx, (images, masks, targetss, img_names) in enumerate(data_loader):
                ...
                output = energy_model.classify(images)
                embeddings.append(output)
            embeddings = torch.cat(embeddings, dim=0)
            # 尝试使用 GPU 加速 KMeans
            kmeans = KMeans(n_clusters=2, random_state=0).fit(embeddings.cpu().numpy())
        ...
        for idx, (images, masks, targetss, img_names) in enumerate(data_loader):
            images = images.to(device)
            targetss = targetss.to(device)
            replay_buffer = init_random(buffer_size, im_sz=images.shape[-1], n_ch=images.shape[1])

            x_fake, _ = sample_q(energy_model, replay_buffer,
                                 n_steps=sgld_steps, sgld_lr=sgld_lr, sgld_std=sgld_std, reinit_freq=reinit_freq,
                                 batch_size=images.shape[0], im_sz=images.shape[-1], n_ch=images.shape[1],
                                 device=device, y=None)
            # 能量方案损失
            out_real = energy_model(images)
            energy_real = out_real[1].mean()
            energy_fake = energy_model(x_fake)[1].mean()
            # print('energy_real:', energy_real.item())
            loss_energy = torch.clamp(-(energy_real - energy_fake), min=0.0)  # 损失函数
            # loss_energy = -(energy_real - energy_fake) # 损失函数

            out_model = energy_model.classify(images)
            probs = F.softmax(out_model, dim=1)
            # counts = torch.zeros(n_classes)
            hssc_loss = HSSCLoss(num_classes=2, tau=1.0, epsilon=0.1)
            ...
            loss_entropy_meter.update(loss_energy.item(), images.size(0))
            loss_cls_meter.update(loss_infoNCE.item(), images.size(0))
            if idx % config.PRINT_FREQ == 0:
                ...
                logger.info(
                    ...
                    f'energy_loss {loss_entropy_meter.val:.4f} ({loss_entropy_meter.avg:.4f})\t'
                    f'classifier_loss {loss_cls_meter.val:.4f} ({loss_cls_meter.avg:.4f})\t'
                )
        # validate_network(data_loader, energy_model, 'shanxi_test')
...
if __name__ == '__main__':
    ...
    for kk in range(len(lrs)):
        ...
        config.OUTPUT = '/disk3/wjr/workspace/sec_proj4/baseline_expeript/TTA/CHOOSEN_200/SLGMS/EFSDA_wclamp/'
        folder_path = config.OUTPUT
        if not os.path.exists(folder_path):
            os.makedirs(folder_path)
        file_name = 'EFSDA_result.txt'
        ...
        with open(file_path, 'a', encoding='utf-8') as file:
            re = f'{args.model_name} {config.BASE_LR}:\n' \
                 ...
                 f'EFSDA global_shanxi_test_avg: {shanxi_test_shot[0]}, shanxi_test_auc: {shanxi_test_shot[-1]}, shanxi_test_acc: {shanxi_test_shot[1]}, shanxi_test_sensitivity: {shanxi_test_shot[2]}, shanxi_test_specificity: {shanxi_test_shot[3]}\n' \
                 f'EFSDA global_guizhou_test_avg: {guizhoutest_shot[0]}, guizhou_test_auc: {guizhoutest_shot[-1]}, guizhou_test_acc: {guizhoutest_shot[1]}, guizhou_test_sensitivity: {guizhoutest_shot[2]}, guizhou_test_specificity: {guizhoutest_shot[3]}\n\n'

            file.write(re)

# f'ORG global_all_test_acc: {avg_test_acc_org} ---> EFSDA global_all_test_acc: {avg_test_acc_tent}\n' \
