    ...
    for epoch in range(config.TRAIN.EPOCHS):
        ...
        for idx, (images, masks, labels, sub_rois, masks_index, imgname) in enumerate(data_loader):
            ...
            if np.any(reliable_mask_ii):
                classifier_loss = nn.CrossEntropyLoss()(outputs_test[reliable_mask_ii, ...],
                                                        pred[reliable_mask_ii, ...])
            else:
            ...
            entropy_loss = torch.mean(Entropy(softmax_out))
            loss = classifier_loss+config.LOCRADIO*rule_loss+config.SMOOTHRADIO*smooth_loss
    ...
    for ii in range(len(radios)):
        for kk in range(len(LOCRADIOS)):
            for jj in range(len(SMOOTHRADIOS)):
                ...
                radio = radios[ii]
                config.RADIO=radio
                num_prototype=num_prototypes[0]
                config.NUM_PROTOTYPES=num_prototype
                config.LOCRADIO=LOCRADIOS[kk]
                config.SMOOTHRADIO=SMOOTHRADIOS[jj]
                config.SMOOTH_EPS = 0
                ...
                file_name = 'shot_biaozhun_reliable_multi_prototypes_glb_loc_reliable_result.txt'
                ...
                with open(file_path, 'a', encoding='utf-8') as file:
                    re = f"{args.model_name} {config.BASE_LR} learnable_para_RADIO:{config.RADIO} NUM_PROTOTYPES:{config.NUM_PROTOTYPES} SMOOTH_EPS:{config.SMOOTH_EPS} JSDnew_sum_rule_weight:{config.LOCRADIO} SMOOTH_loss_weight:{config.SMOOTHRADIO}\n"
                        # --- Shanxi ORG ---
                    re += "=== Shanxi Test (ORG) ===\n"
                    # global
                    g = shanxi_test_org[0]
