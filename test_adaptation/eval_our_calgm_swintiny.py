...
class ResNet50WithFeats(nn.Module):
    def __init__(self, pretrained_cfg):
        super().__init__()
    ...
    def forward(self, x):
        # 提 backbone 特征
        feats = self.model.backbone(x)              # 一般是 tuple
...
def main(config, pt_path, logger_path):
    ...
    if args.model_name=='resnet50':
        ...
        pretrained_cfg = '/disk3/wjr/workspace/sec_nejm/nejm_baseline_expeript_new/allshanxi_sick_health/writer_analyze/512/allstage1_health1_1/selected_dcm_saomiao_900_health308_sick_592/new_contra_20250626/swintiny/wmask_nosubcls_nodropout2/swintiny_lr0.0001/model_ema_best_auc_shanxi_val.pth'
        ...
        model = SwinTinyWithFeats(pretrained_cfg)
        ...
        model = ViTWithFeats(pretrained_cfg)
    ...
    if os.path.isfile(ckp_path):
        ...
        # ckp_path2 = os.path.join(logger_path,'guizhou_test.pth')
        # state_dict = torch.load(ckp_path2, map_location='cpu', weights_only=False)['model']
        # msg = model.load_state_dict(state_dict, strict=False)
    ...
    for ii in range(len(radios)):
        for jj in range(len(num_prototypes)):
            for kk in range(len(FRADIOS)):
                ...
                config.OUTPUT = '/disk2/wxr/vi_feiqu_baseline/baseline_expeript/TTA/CALGM/ablation/revision/FUSION_radio/swintiny'
