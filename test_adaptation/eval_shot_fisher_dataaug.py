...
def train_one_epoch(config, model, data_loader, dataloader_small):
    ...
    for epoch in range(config.TRAIN.EPOCHS):
        ...
        for idx, (images, masks, targetss, img_names) in enumerate(data_loader):
            ...
            outputs_test_aug = model(tta_transforms(images))
            is_tuple = isinstance(outputs_test_aug, tuple)
            if is_tuple:
                outputs_test_aug = outputs_test_aug[0]
            ...
            classifier_loss_aug = nn.CrossEntropyLoss()(outputs_test_aug, pred)
            softmax_out = nn.Softmax(dim=1)(outputs_test)
            entropy_loss = torch.mean(Entropy(softmax_out))
            loss = classifier_loss+classifier_loss_aug
            ...
            loss_aug_meter.update(classifier_loss_aug.item(), images.size(0))
            ...
            if idx % config.PRINT_FREQ == 0:
                ...
                logger.info(
                    ...
                    f'classifier_loss_aug {loss_aug_meter.val:.4f} ({loss_aug_meter.avg:.4f})\t'
...
if __name__ == '__main__':
    ...
    for kk in range(len(lrs)):
        ...
        for jj in range(len(radios)):
            ...
            file_name = 'shot_dataaug_result.txt'
            ...
            with open(file_path, 'a', encoding='utf-8') as file:
                re = f'{args.model_name} {config.BASE_LR}:\n' \
                     ...
                     f'SHOT dataaug TRAIN_FEA_fisher_pesudo_{radio} global_shanxi_test_avg: {shanxi_test_shot[0]}, shanxi_test_auc: {shanxi_test_shot[-1]}, shanxi_test_acc: {shanxi_test_shot[1]}, shanxi_test_sensitivity: {shanxi_test_shot[2]}, shanxi_test_specificity: {shanxi_test_shot[3]}\n' \
                     f'SHOT dataaug TRAIN_FEA_fisher_pesudo_{radio} global_guizhou_test_avg: {guizhoutest_shot[0]}, guizhou_test_auc: {guizhoutest_shot[-1]}, guizhou_test_acc: {guizhoutest_shot[1]}, guizhou_test_sensitivity: {guizhoutest_shot[2]}, guizhou_test_specificity: {guizhoutest_shot[3]}\n\n'
