...
def main(config, pt_path, logger_path):
    ...
    if os.path.isfile(ckp_path):
        ...
        if args.model_name == "slgms":
            ...
            msg = model.load_state_dict(state_dict, strict=False)
        model.to(device)
        shanxi_test_org = validate_network_org(data_loader_test, model, 'shanxi_test_org',
                                               save_path=logger_path + '/analyze/')
        guizhou_test_org = validate_network_org(data_loader_test2, model, 'guizhou_test_org',
                                                save_path=logger_path + '/analyze/')
...
if __name__ == '__main__':
    ...
    for kk in range(len(lrs)):
        ...
        config.OUTPUT = '/disk3/wjr/workspace/sec_proj4/baseline_expeript/TTA/CHOOSEN_200/SLGMS/Adacontrast/'
