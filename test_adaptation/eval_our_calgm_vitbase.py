...
def parse_option():
    ...
    parser.add_argument('--output', default='/disk2/wxr/vi_feiqu_baseline/baseline_expeript/selected_dcm_saomiao_900_health308_sick_592/224/contra_20250906/', type=str, metavar='PATH',
...
class ResNet50WithFeats(nn.Module):
    def __init__(self, pretrained_cfg=None):
        super().__init__()
        self.model = get_model(
            'resnet50_8xb32_in1k',
            pretrained=pretrained_cfg if pretrained_cfg is not None else False,
            backbone=dict(drop_path_rate=0.)
        )
        self.model.head.fc = nn.Linear(2048, 2)
...
class ViTWithFeats(nn.Module):
    def __init__(self, pretrained_cfg=None):
        ...
        self.model.head.layers.head = nn.Linear(768, 2)
    ...
    def forward(self, x):
        ...
        logits = self.model.head.layers.head(feats)
        return logits, feats


class SwinTinyWithFeats(nn.Module):
    def __init__(self, pretrained_cfg=None):
        super().__init__()
        self.model = get_model(
        ...
        self.model.head.fc = nn.Linear(768, 2, bias=True)

        if pretrained_cfg is not None:
            ckpt = torch.load(pretrained_cfg, map_location='cpu', weights_only=False)
            ...
            print('Loaded Swin pretrained:', msg)
...
def load_ckpt_state_dict(ckp_path, model_name=None):
    ckpt = torch.load(ckp_path, map_location='cpu', weights_only=False)
    if not isinstance(ckpt, dict):
        return ckpt

    if model_name == "slgms" and 'teacher' in ckpt:
...
def load_model_weights(model, ckp_path, model_name=None, logger=None):
    state_dict = load_ckpt_state_dict(ckp_path, model_name=model_name)

    if model_name == "pneullm":
        model_dict = model.state_dict()
        state_dict = {k: v for k, v in state_dict.items() if k in model_dict}

    msg = model.load_state_dict(state_dict, strict=False)
    print(f"Load checkpoint msg ({model_name}):", msg)
    if logger is not None:
        logger.info(f"Load checkpoint msg ({model_name}): {msg}")
    return model


def create_model_by_name(args, config):
    if args.model_name == 'resnet50':
        pretrained_cfg = '/disk3/wjr/workspace_lyt/pretrained_weight/mmpre/resnet50_8xb32_in1k_20210831-ea4938fc.pth'
        model = ResNet50WithFeats(pretrained_cfg)
        ...
        model = SwinTinyWithFeats(None)

    elif args.model_name == "vit-base":
        model = ViTWithFeats(None)
...
def main(config, pt_path, logger_path):
    ...
    data_loader_val, data_loader_test, data_loader_test2, data_loader_prototype, data_loader_test_small, data_loader_test2_small = build_phe_loader(args)

    logger.info(f"Creating model:{args.model_name}")
    model = create_model_by_name(args, config)

    ckp_path = pt_path
    if os.path.isfile(ckp_path):
        model = load_model_weights(model, ckp_path, model_name=args.model_name, logger=logger)

        logger.info("TTA ON SHANXITEST..........................................")
        model = train_one_epoch(config, model, data_loader_prototype, data_loader_test, data_loader_test_small)
        shanxi_test_shot = validate_network_glb(data_loader_test, model, 'shanxi_test', save_path=logger_path + '/analyze/')

        end_time = time.time()
        total_inference_time = end_time - start_time
        print(f"{args.model_name}: Total inference time (including data loading): {total_inference_time:.4f} seconds")
        logger.info(f"{args.model_name}: Total inference time (including data loading): {total_inference_time:.4f} seconds")
        ...
        # 重新加载原始 checkpoint，做 org 测试
        model = create_model_by_name(args, config)
        model = load_model_weights(model, ckp_path, model_name=args.model_name, logger=logger)
        model.to(device)

        shanxi_test_org = validate_network_glb(data_loader_test, model, 'shanxi_test_org', save_path=logger_path + '/analyze/')
        guizhou_test_org = validate_network_glb(data_loader_test2, model, 'guizhou_test_org', save_path=logger_path + '/analyze/')

        logger.info("TTA ON GUIZHOUTEST..........................................")
        model = train_one_epoch(config, model, data_loader_prototype, data_loader_test2, data_loader_test2_small)
        guizhoutest_shot = validate_network_glb(data_loader_test2, model, 'guizhou_test', save_path=logger_path + '/analyze/')
...
@torch.no_grad()
def validate_network_glb(val_loader, model, valdata='shanxi_val', save_path=''):
    model.eval()
    header = 'Test:'
    # 初始化输出和目标列表
    all_outputs = []
    ...
    for idx, (images, masks, labels, sub_rois, masks_index, imgname) in enumerate(val_loader):
        ...
        for bb in range(prob.shape[0]):
            ...
            if aa == 0:
                Global_pred_global_labels = 'Sick'
            else:
                Global_pred_global_labels = 'Health'
            result["Global Pred Labels"] = Global_pred_global_labels
            aa = torch.argmax(global_prob[bb, :])
            if aa == 0:
                local_pred_global_labels = 'Sick'
            else:
                local_pred_global_labels = 'Health'
            result["Local Pred Labels"] = local_pred_global_labels
            aa = torch.argmax((global_prob[bb, :] + prob[bb, :]) / 2)
            if aa == 0:
                global_local_pred_global_labels = 'Sick'
            else:
                global_local_pred_global_labels = 'Health'
            result["Global_Local Pred Labels"] = global_local_pred_global_labels
            result["Global Pred logits"] = '%.4f' % prob[bb, aa].cpu().detach().numpy()
            result["Local Pred logits"] = '%.4f' % global_prob[bb, aa].cpu().detach().numpy()
            result["Global_Local Pred logits"] = '%.4f' % ((global_prob[bb, aa] + prob[bb, aa]) / 2).cpu().detach().numpy()
    ...
    df_data_results.to_csv(save_path + '/' + valdata + '_dataprob_result.csv', index=False)
...
def train_one_epoch(config, model, data_loader_prototype, data_loader, dataloader_small):
    ...
    if len(structure_fisher) == 0:
        raise ValueError("structure_fisher is empty. No parameters were hooked for Fisher computation.")
    ...
    logger.info('Trainable parameters:', names)
...
if __name__ == '__main__':
    ...
    lrs = [8e-5]
    radios = [15]
    # num_prototypes=[10,9,7,2,3,1,5,6,8]
    num_prototypes = [4]
    LOCRADIOS = [0.6]
    FRADIOS = [0.5]
    for ii in range(len(radios)):
        for jj in range(len(num_prototypes)):
            for kk in range(len(FRADIOS)):
                args = parse_option()
                ...
                radio = radios[ii]
                config.RADIO = radio
                config.FRADIOS = FRADIOS[kk]
                num_prototype = num_prototypes[jj]
                config.NUM_PROTOTYPES = num_prototype
                config.LOCRADIO = LOCRADIOS[0]
                config.OUTPUT = '/disk2/wxr/vi_feiqu_baseline/baseline_expeript/TTA/CALGM/ablation/revision/FUSION_radio/vit-base'
                ...
                pt_path = '/disk3/wjr/workspace/sec_nejm/nejm_baseline_expeript_new/allshanxi_sick_health/writer_analyze/512/allstage1_health1_1/selected_dcm_saomiao_900_health308_sick_592/new_contra_20250626/swintiny/wmask_nosubcls_nodropout2/swintiny_lr0.0001/model_ema_best_auc_shanxi_val.pth'
                ...
                logger_path = os.path.join(config.OUTPUT,
                                           f'{config.DATA.BATCH_SIZE}batch_{config.BASE_LR}lr_{config.FRADIOS}FRADIOS_{config.RADIO}lpara_{config.NUM_PROTOTYPES}prototypes_{config.LOCRADIO}rule_JSD')

                if not os.path.exists(logger_path):
                    os.makedirs(logger_path)
                if not os.path.exists(logger_path + '/analyze'):
                    os.mkdir(logger_path + '/analyze')
                ...
                with open(file_path, 'a', encoding='utf-8') as file:
                    ...
                    re += f"LOCAL_GLOBAL→ avg: {lg[0]:.4f}, acc: {lg[1]:.4f}, sen: {lg[2]:.4f}, spe: {lg[3]:.4f}, auc: {lg[4]:.4f}\n\n"
                    re += f'total_inference_time: {total_inference_time} seconds\n' \
