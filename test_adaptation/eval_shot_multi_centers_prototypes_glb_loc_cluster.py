...
def train_one_epoch(config, model, data_loader_prototype, data_loader, dataloader_small):
    ...
    mem_label, reliable_mask,imgnames = obtain_multi_centers_multi_prototype_label(data_loader_prototype, dataloader_small, model, NUM_PROTO = config.NUM_PROTOTYPES)
    mem_label = torch.from_numpy(mem_label).to(device)
    structure_fisher = compute_structure_fisher_scores_pesudo(model, dataloader_small, mem_label, device)
    threshold = np.percentile(list(structure_fisher.values()), 100 - config.RADIO)  # 冻结 bottom 30%
    names = []
    for name, param in model.named_parameters():
        layer_key = '.'.join(name.split('.')[:-1])
        if structure_fisher.get(layer_key, 0) < threshold:
    ...
    for epoch in range(config.TRAIN.EPOCHS):
        ...
        for idx, (images, masks, labels, sub_rois, masks_index, imgname) in enumerate(data_loader):
            ...
            local_global_losses=[]
            for ff in range(6):
                fea_lossii = torch.sum(-glb_fea * F.log_softmax(sub_feas2[:,ff,...], dim=-1), dim=-1)
            ...
            mask_sorted_sick = torch.zeros_like(local_global_losses)
            selected_indices_sick=[]
            selected_indices_health = []
            for ii in range(local_global_losses.shape[1]):
                label_locali = score_mean_global[ii]
                # 类别1为正常，求概率最小的局部分区值参与分类任务，其余生病求对应类别局部块内概率最大的局部分区参与分类任务
                if label_locali != 1:
                    aa = torch.argmax(subregions_outputs2[ii,...],dim=-1)
                    cc = local_global_losses[:, ii]
                    # 患病的求局部与全局最相近的2个分区参与分类任务
                    sorted_values, sorted_indices = torch.sort(cc, descending=False)
                    top_biggest = sorted_indices[0]
                    second_biggest = sorted_indices[1]
                    if aa[top_biggest] != 1:
                        selected_indices_sick.append([top_biggest, ii])
                    if aa[second_biggest] != 1:
                        selected_indices_sick.append([second_biggest, ii])
                else:
                    aa = torch.argmax(subregions_outputs2[ii, ...], dim=-1)
                    cc = local_global_losses[:, ii]
                    # 健康的求局部与全局最相近的前五个分区随机挑选两个参与分类任务
                    sorted_values, sorted_indices = torch.sort(cc, descending=False)
                    random_choice = random.sample([0, 1, 2, 3, 4], 2)
                    third_biggest = sorted_indices[random_choice[0]]
                    if aa[third_biggest] ==1:
                        selected_indices_health.append([third_biggest, ii])

                    four_biggest = sorted_indices[random_choice[1]]
                    if aa[four_biggest] == 1:
                        selected_indices_health.append([four_biggest, ii])

            for (r, c) in selected_indices_sick:
                mask_sorted_sick[r, c] = 1.0

            for (r, c) in selected_indices_health:
                mask_sorted_sick[r, c] = 1.0
            if mask_sorted_sick.sum() < 1:
                cls_loss_local = torch.tensor(0.0, device=device)
            else:
                score_mean_global_expanded = score_mean_global.unsqueeze(1).repeat(1, 6).flatten()  # (N, 6)
                mask_sorted_sick = mask_sorted_sick.flatten(0, 1)  # (N×6, C)
                cls_loss_local = (nn.CrossEntropyLoss(reduction='none')(subregions_outputs,
                                                                        score_mean_global_expanded) * mask_sorted_sick).sum() / mask_sorted_sick.sum()

            V_prob = local2global_prob(subregions_outputs2).detach()
            ...
            loss = classifier_loss+rule_loss
            ...
            if idx % config.PRINT_FREQ == 0:
                ...
                logger.info(
                    ...
                    f'global classifier_loss {loss_glb_cls_meter.val:.4f} ({loss_glb_cls_meter.avg:.4f})\t'
                    f'local classifier_loss {loss_loc_cls_meter.val:.4f} ({loss_loc_cls_meter.avg:.4f})\t'
                    f'rule_loss {loss_con_meter.val:.4f} ({loss_con_meter.avg:.4f})\t'
...
def obtain_multi_centers_multi_prototype_label(loader_prototype, loader, model, NUM_PROTO = 3, INIT_WEIGHT = 0.5, valdata = ''):
    ...
    with torch.no_grad():
        for idx, (images, masks, labels, sub_rois, masks_index, imgname) in enumerate(loader_prototype):
            ...
            if isinstance(masks[0], list):
                ...
                sample_masks = masks[0].to(device)
            inps = (samples * sample_masks[0]).to(device)
            outputs, feas = model(inps)
            if start_test:
            ...
            sub_regions_images = torch.stack(images[1:7])  # 提取索引1-6，直接堆叠
            sub_regions_masks = torch.stack(masks[1:7])
            sub_regions_labels = torch.stack(labels[1:7])

            # 调整维度+展平（适用于任意通道数/尺寸的张量）
            subregions_imgs = sub_regions_images.permute(1, 0, 2, 3, 4).flatten(0, 1).to(device)
            subregions_masks = sub_regions_masks.permute(1, 0, 2, 3, 4).flatten(0, 1).to(device)
            subregions_labels = sub_regions_labels.permute(1, 0, 2).flatten(0, 1).to(device).squeeze(-1)
            subinps = (subregions_imgs * subregions_masks).to(device)
            suboutputs, subfeas = model(subinps)
            if start_sub_test:
                all_subprototype_fea = subfeas.float().cpu()
                all_subprototype_output = suboutputs.float().cpu()
                all_subprototype_label = subregions_labels.float().cpu()
                start_sub_test = False
            else:
                all_subprototype_fea = torch.cat((all_subprototype_fea, subfeas.float().cpu()), 0)
                all_subprototype_output = torch.cat((all_subprototype_output, suboutputs.float().cpu()), 0)
                all_subprototype_label = torch.cat((all_subprototype_label, subregions_labels.float().cpu()), 0)

    _, sub_predict = torch.max(all_subprototype_output, 1)
    all_subprototype_predict=sub_predict.cpu().numpy()
    all_subprototype_label2=all_subprototype_label.clone()
    all_subprototype_label2[(all_subprototype_label2 == 0) | (all_subprototype_label2 == 1) | (
            all_subprototype_label2 == 2)] = 0
    all_subprototype_label2[(all_subprototype_label2 == 3) | (all_subprototype_label2 == 4)] = 1
    ...
    all_subprototype_fea = (all_subprototype_fea.t() / torch.norm(all_subprototype_fea, p=2, dim=1)).t()
    final_subprototype_fea = all_subprototype_fea.float().cpu().numpy()
    anchor_subfea_0 = final_subprototype_fea[all_subprototype_label == 0]  # (N0, D)
    anchor_subfea_1 = final_subprototype_fea[all_subprototype_label == 1]  # (N1, D)
    anchor_subfea_2 = final_subprototype_fea[all_subprototype_label == 2]  # (N1, D)
    anchor_subfea_3 = final_subprototype_fea[all_subprototype_label == 3]  # (N1, D)
    anchor_subfea_4 = final_subprototype_fea[all_subprototype_label == 4]  # (N1, D)
    ...
    with torch.no_grad():
        for idx, (images, masks, labels, sub_rois, masks_index, imgname) in enumerate(loader):
            ...
            suboutputs, subfeas = model(subinps)
            if start_sub_test:
                all_sub_fea = subfeas.float().cpu()
                all_sub_output = suboutputs.float().cpu()
                # all_sub_label = subregions_labels.float().cpu()
                start_sub_test = False
            else:
                all_sub_fea = torch.cat((all_sub_fea, subfeas.float().cpu()), 0)
                all_sub_output = torch.cat((all_sub_output, suboutputs.float().cpu()), 0)
                # all_subprototype_label = torch.cat((all_subprototype_label, subregions_labels.float().cpu()), 0)
    ...
    '''局部肺区聚类处理'''
    all_sub_output = nn.Softmax(dim=1)(all_sub_output)
    _, sub_predict = torch.max(all_sub_output, 1)
    all_sub_fea = (all_sub_fea.t() / torch.norm(all_sub_fea, p=2, dim=1)).t()
    final_sub_fea = all_sub_fea.float().cpu().numpy()
    dd_sub_anchor_0 = cdist(final_sub_fea, anchor_subfea_0, metric='cosine')
    dd_sub_anchor_1 = cdist(final_sub_fea, anchor_subfea_1, metric='cosine')
    dd_sub_anchor_2 = cdist(final_sub_fea, anchor_subfea_2, metric='cosine')
    dd_sub_anchor_3 = cdist(final_sub_fea, anchor_subfea_3, metric='cosine')
    dd_sub_anchor_4 = cdist(final_sub_fea, anchor_subfea_4, metric='cosine')
        ...
        for ii in range(len(radios)):
            for jj in range(len(num_prototypes)):
                ...
                with open(file_path, 'a', encoding='utf-8') as file:
                    re = f'{args.model_name} {config.BASE_LR} RADIO:{config.RADIO} NUM_PROTOTYPES:{config.NUM_PROTOTYPES}:\n' \
                         ...
                         f'SHOT TRAIN_rule_loss_FEA_multi_prototype_{config.NUM_PROTOTYPES} global_shanxi_test_avg: {shanxi_test_shot[0]}, shanxi_test_auc: {shanxi_test_shot[-1]}, shanxi_test_acc: {shanxi_test_shot[1]}, shanxi_test_sensitivity: {shanxi_test_shot[2]}, shanxi_test_specificity: {shanxi_test_shot[3]}\n' \
                         f'SHOT TRAIN_rule_loss_FEA_multi_prototype_{config.NUM_PROTOTYPES} global_guizhou_test_avg: {guizhoutest_shot[0]}, guizhou_test_auc: {guizhoutest_shot[-1]}, guizhou_test_acc: {guizhoutest_shot[1]}, guizhou_test_sensitivity: {guizhoutest_shot[2]}, guizhou_test_specificity: {guizhoutest_shot[3]}\n\n'
