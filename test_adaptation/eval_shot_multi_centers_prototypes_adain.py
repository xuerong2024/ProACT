...
def build_phe_loader(args):
    ...
    dataset_test2 = Shanxi_wmask_Dataset('/disk3/wjr/dataset/nejm/guizhoudataset/seg_rec_img_1024','/disk3/wjr/dataset/nejm/guizhoudataset/seg_rec_mask_1024',
                                   txtpath='/disk3/wjr/dataset/nejm/guizhoudataset/guizhou_one_chinese_biaozhun.txt',
                                   data_transform=val_transform1)
    dataset_test3 = Shanxi_wmask_Dataset(args.data_root + 'seg_rec_img_1024', args.data_root + '/seg_rec_mask_1024',
...
def compute_structure_fisher_scores_pesudo(model, support_loader, mem_label, device):
    ...
    for idx, (inp, mask, target, imgname) in enumerate(support_loader):
        inp = (inp * mask).to(device)
        target = target.to(device)
...
def train_one_epoch(config, model, data_loader_prototype, data_loader, dataloader_small):
    mem_label, imgnames = obtain_multi_centers_multi_prototype_label(data_loader_prototype, dataloader_small, model, NUM_PROTO = config.NUM_PROTOTYPES)
    ...
    for epoch in range(config.TRAIN.EPOCHS):
        ...
        mem_label, imgnames = obtain_multi_centers_multi_prototype_label(data_loader_prototype, data_loader, model, NUM_PROTO = config.NUM_PROTOTYPES, valdata = f'{config.NUM_PROTOTYPES}prototypes_2centers')
        mem_label = torch.from_numpy(mem_label).to(device)
    ...
    mem_label, imgnames = obtain_multi_centers_multi_prototype_label(data_loader_prototype, data_loader, model, NUM_PROTO = config.NUM_PROTOTYPES)
    return model
...
def obtain_multi_centers_multi_prototype_label(loader_prototype, loader, model, NUM_PROTO = 3, INIT_WEIGHT = 0.5, valdata = ''):
    ...
    img_names = []
    with torch.no_grad():
        for idx, (images, masks, targetss, imaname) in enumerate(loader_prototype):
            images = (images * masks).to(device)
            outputs, feas, feas_spa = model(images)
            for name in imaname:
                img_names.append(name)
            if start_test:
                ...
                all_prototype_fea_spa = feas_spa.float().cpu()
                start_test = False
            else:
                all_prototype_fea = torch.cat((all_prototype_fea, feas.float().cpu()), 0)
                all_prototype_fea_spa = torch.cat((all_prototype_fea_spa, feas_spa.float().cpu()), 0)
    ...
    # all_prototype_predict=torch.argmax(all_prototype_output, dim=-1)
    # all_prototype_output = nn.Softmax(dim=1)(all_prototype_output).cpu().numpy()
    ...
    # aff_prototype = all_prototype_output
    # initc_soft = aff_prototype.T @ final_prototype_fea
    # initc_prototype = initc_soft / (1e-8 + aff_prototype.sum(axis=0)[:, None])

    # all_prototype_fea = (all_prototype_fea.t() / torch.norm(all_prototype_fea, p=2, dim=1)).t()
    # final_prototype_fea = all_prototype_fea.float().cpu().numpy()
    # # all_prototype_label
    # K=outputs.shape[-1]
    # aff_prototype = np.eye(K)[all_prototype_label.long().cpu().numpy()]
    # initc_prototype = aff_prototype.transpose().dot(final_prototype_fea)
    start_test = True
    with torch.no_grad():
        for idx, (images, masks, targetss, imaname) in enumerate(loader):
            for name in imaname:
            ...
            if start_test:
                all_fea = feas.float().cpu()
                all_fea_spa = feas_spa.float().cpu()
                all_output = outputs.float().cpu()
                all_label = targetss[:, 1].float().cpu()
                start_test = False
            else:
                all_fea = torch.cat((all_fea, feas.float().cpu()), 0)
                all_fea_spa = torch.cat((all_fea_spa, feas_spa.float().cpu()), 0)
                all_output = torch.cat((all_output, outputs.float().cpu()), 0)
                all_label = torch.cat((all_label, targetss[:, 1].float().cpu()), 0)

    all_prototype_fea_spa2=AdaIN(all_prototype_fea_spa, all_fea_spa)
    a=1
    # -----------------------------
    # Softmax prediction
    # -----------------------------
    ...
    for cls in labelset:
        ...
        # 融合 softmax prototype（SHOT 原味）
        for c in centers:
            # fused_center0 = INIT_WEIGHT * initc_soft[cls] + (1 - INIT_WEIGHT) * c
    ...
    for kk in range(len(lrs)):
        for ii in range(len(radios)):
            for jj in range(len(num_prototypes)):
                ...
                file_name = 'shot_multi_centers_multi_prototypes_result.txt'
                ...
                with open(file_path, 'a', encoding='utf-8') as file:
                    re = f'{args.model_name} {config.BASE_LR} RADIO:{config.RADIO} NUM_PROTOTYPES:{config.NUM_PROTOTYPES}:\n' \
                         ...
                         f'SHOT TRAIN_FEA_multi_prototype_{config.NUM_PROTOTYPES} global_shanxi_test_avg: {shanxi_test_shot[0]}, shanxi_test_auc: {shanxi_test_shot[-1]}, shanxi_test_acc: {shanxi_test_shot[1]}, shanxi_test_sensitivity: {shanxi_test_shot[2]}, shanxi_test_specificity: {shanxi_test_shot[3]}\n' \
                         f'SHOT TRAIN_FEA_multi_prototype_{config.NUM_PROTOTYPES} global_guizhou_test_avg: {guizhoutest_shot[0]}, guizhou_test_auc: {guizhoutest_shot[-1]}, guizhou_test_acc: {guizhoutest_shot[1]}, guizhou_test_sensitivity: {guizhoutest_shot[2]}, guizhou_test_specificity: {guizhoutest_shot[3]}\n\n'
