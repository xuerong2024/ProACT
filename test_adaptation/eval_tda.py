...
def main(config, ptname):
    ...
    if os.path.isfile(ckp_path):
        if args.model_name == "pneullm":
            ...
            msg = model.load_state_dict(state_dict, strict=False)
        shanxi_test_tda = run_test_tda(data_loader_test, model, pos_shot_capacity=20, pos_alpha=2, pos_beta=5,
                                       val_index='shanxi_test')
        guizhou_test_tda = run_test_tda(data_loader_test2, model, pos_shot_capacity=20, pos_alpha=2, pos_beta=5,
                                        val_index='guizhou_test')

        shanxi_test_org = validate_network(data_loader_test, model, val_index='shanxi_test')
        guizhou_test_org = validate_network(data_loader_test2, model, val_index='guizhou_test')
        return shanxi_test_org, guizhou_test_org, shanxi_test_tda, guizhou_test_tda
...
@torch.no_grad()
def run_test_tda(loader, model, pos_shot_capacity=3, pos_alpha=2, pos_beta=5, val_index='shanxi_test'):
    model.eval()
    # 初始化输出和目标列表
    all_outputs = []
    all_targets = []
    ...
    with torch.no_grad():
        ...
        for i, (inp, mask, target, imgname) in enumerate(tqdm(loader, desc='Processed test images: ')):
            ...
            inp = (inp * mask).to(device)
            target = target.to(device)
            _clsout, _, _, image_features = model(inp)
        ...
        for i, (inp, mask, target, imgname) in enumerate(tqdm(loader, desc='Processed test images: ')):
            ...
            pred = int(_clsout.topk(1, 1, True, True)[1].t()[0])
            if pos_cache:
                final_logits = compute_cache_logits(image_features, pos_cache, _clsout.shape[-1], pos_alpha,
                                                            pos_beta)
...
if __name__ == '__main__':
    ...
    with open(file_path, 'a', encoding='utf-8') as file:
        re = f'{args.model_name} {config.BASE_LR}:\n' \
             ...
             f'TDA global_shanxi_test_avg: {shanxi_test_shot[0]}, shanxi_test_auc: {shanxi_test_shot[-1]}, shanxi_test_acc: {shanxi_test_shot[1]}, shanxi_test_sensitivity: {shanxi_test_shot[2]}, shanxi_test_specificity: {shanxi_test_shot[3]}\n' \
             f'TDA global_guizhou_test_avg: {guizhoutest_shot[0]}, guizhou_test_auc: {guizhoutest_shot[-1]}, guizhou_test_acc: {guizhoutest_shot[1]}, guizhou_test_sensitivity: {guizhoutest_shot[2]}, guizhou_test_specificity: {guizhoutest_shot[3]}\n\n'
