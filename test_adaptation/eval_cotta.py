# CoTTA: Continual Test-Time Adaptation, CVPR2022, 随机训练一小部分模型参数
import os
import time
import json
import random
...
def parse_option():
    ...
    parser.add_argument('--output', default='/disk3/wjr/workspace/sec_proj4/baseline_expeript/selected_dcm_saomiao_900_health308_sick_592/224/contra_20260323/', type=str, metavar='PATH',
...
def adaptive_calculate_metrics(all_outputs, all_targets_onehot,threshold=None, valdata='shanxi_test',num_classes=2, plot_roc=True, save_fpr_tpr=True, save_path=''):
    ...
    for i in range(1):
        ...
        if plot_roc:
            ...
            im_save_path = save_path + '/' + valdata + '_roc_map.pdf'
            plt.savefig(im_save_path, dpi=500, bbox_inches='tight')  # 保存图像，设置分辨率和裁剪边距
            # # # 显示
            # # plt.show()
            # # 自动关闭图像
            plt.close()
...
def train_one_epoch(config, model, data_loader, val_index='', save_path=''):
    ...
    for epoch in range(1):
        ...
        for idx, (images, masks, targetss, img_names) in enumerate(data_loader):
            ...
            if is_tuple:
                outputs = outputs[0]
            # Teacher Prediction
            anchor_output=model_anchor(images)
            is_tuple = isinstance(anchor_output, tuple)
            if is_tuple:
                anchor_output = anchor_output[0]
            anchor_prob = torch.nn.functional.softmax(anchor_output, dim=1).max(1)[0]
            standard_ema = model_ema(images)
            is_tuple = isinstance(standard_ema, tuple)
            if is_tuple:
                standard_ema = standard_ema[0]
            ...
            if to_aug:
                for i in range(N):
                    outputs_ = model_ema(tta_transforms(images)).detach()
                    is_tuple = isinstance(outputs_, tuple)
                    if is_tuple:
                        outputs_ = outputs_[0]
            ...
            prob = torch.nn.functional.softmax(outputs_ema, dim=1).float()
            for bb in range(prob.shape[0]):
                result = {}
        ...
        all_targets = np.array(all_targets)
        df_data_results = pd.DataFrame(data_results)
        Path(save_path + '/' + 'analyze').mkdir(parents=True, exist_ok=True)
        ...
        save_state = {'model': model_ema.state_dict()}
...
if __name__ == '__main__':
    ...
    for kk in range(len(lrs)):
        ...
        config.OUTPUT = '/disk3/wjr/workspace/sec_proj4/baseline_expeript/TTA/CHOOSEN_200/CALGM/COTTA/'
        folder_path = config.OUTPUT
        if not os.path.exists(folder_path):
            os.makedirs(folder_path)
        file_name = 'cotta_result.txt'
        ...
        with open(file_path, 'a', encoding='utf-8') as file:
            re = f'{args.model_name} {config.BASE_LR}:\n' \
                 ...
                 f'COTTA global_shanxi_test_avg: {shanxi_test_shot[0]}, shanxi_test_auc: {shanxi_test_shot[-1]}, shanxi_test_acc: {shanxi_test_shot[1]}, shanxi_test_sensitivity: {shanxi_test_shot[2]}, shanxi_test_specificity: {shanxi_test_shot[3]}\n' \
                 f'COTTA global_guizhou_test_avg: {guizhoutest_shot[0]}, guizhou_test_auc: {guizhoutest_shot[-1]}, guizhou_test_acc: {guizhoutest_shot[1]}, guizhou_test_sensitivity: {guizhoutest_shot[2]}, guizhou_test_specificity: {guizhoutest_shot[3]}\n\n'
            # f'ORG global_all_test_acc: {avg_test_acc_org} ---> COTTA global_all_test_acc: {avg_test_acc_tent}\n' \
