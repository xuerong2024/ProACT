'''多中心原型ECCV2022 Bmd: A general class-balanced multicentric dynamic prototype strategy for source-free domain adaptation'''
...
def compute_structure_fisher_scores_pesudo(model, support_loader, mem_label, device):
    ...
    for name, param in model.named_parameters():
        if 'head.' not in name and param.requires_grad:
...
if __name__ == '__main__':
    ...
    for kk in range(len(lrs)):
        ...
        for jj in range(len(radios)):
            ...
            file_name = 'shot_result_max_wallpara.txt'
