...
def train_one_epoch(config, model, data_loader, dataloader_small):
    mem_label, imgnames = obtain_multi_prototype_label(dataloader_small, model, NUM_PROTO = config.NUM_PROTOTYPES)
    ...
    for epoch in range(config.TRAIN.EPOCHS):
        ...
        mem_label, imgnames = obtain_multi_prototype_label(data_loader, model, NUM_PROTO = config.NUM_PROTOTYPES, valdata = f'{config.NUM_PROTOTYPES}prototypes')
        mem_label = torch.from_numpy(mem_label).to(device)
    ...
    mem_label, imgnames = obtain_multi_prototype_label(data_loader, model, NUM_PROTO = config.NUM_PROTOTYPES)
    return model
    ...
    for kk in range(len(lrs)):
        for ii in range(len(radios)):
            for jj in range(len(num_prototypes)):
                ...
                file_name = 'shot_multi_prototypes_result.txt'
