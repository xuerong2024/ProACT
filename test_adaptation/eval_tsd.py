'''TSD [cvpr2023] Feature alignment and uniformity for test time adaptation ,CVPR2023'''
import os
import time
import json
import random
...
def main(config, ptname):
    ...
    if os.path.isfile(ckp_path):
        ...
        shanxi_test_shot = train_one_epoch(config, model, data_loader_test, val_index='shanxi_test')
        ...
        guizhoutest_shot = train_one_epoch(config, model, data_loader_test2, val_index='guizhou_test')
...
def select_supports(ent, labels, supports, filter_K=100, num_classes=2):
    ent_s = ent
...
def knn_affinity(X, knn):
    ...
    return W
def train_one_epoch(config, model, data_loader, num_classes=2, val_index=''):
    s_warmup_supports = model.head.fc.weight.data.detach()
    s_warmup_prob = model.head.fc(s_warmup_supports)
    s_warmup_ent = softmax_entropy(s_warmup_prob)
    s_warmup_labels = F.one_hot(s_warmup_prob.argmax(1), num_classes=num_classes).float()
    s_warmup_scores = F.softmax(s_warmup_prob, 1)
    s_supports = s_warmup_supports.data.to(device)
    s_labels = s_warmup_labels.data.to(device)
    s_ent = s_warmup_ent.data.to(device)
    s_scores = s_warmup_scores.data
    ...
    for epoch in range(1):
        ...
        for idx, (images, masks, targetss, img_names) in enumerate(data_loader):
            ...
            outputs_test, fea_test = model(images)
            yhat = F.one_hot(outputs_test.argmax(1), num_classes=num_classes).float()
            ent = softmax_entropy(outputs_test)
            scores = F.softmax(outputs_test, 1)
            with torch.no_grad():
                s_supports = torch.cat([s_supports, fea_test])
                s_labels = torch.cat([s_labels, yhat])
                s_ent = torch.cat([s_ent, ent])
                s_scores = torch.cat([s_scores, scores])
                supports, labels = select_supports(s_ent, s_labels, s_supports, filter_K=100)
                supports = F.normalize(supports, dim=1)
                weights = (supports.T @ (labels))

            dist, loss_enp = prototype_loss(fea_test, weights.T, scores, use_hard=False)

            loss_local = topk_cluster(fea_test.detach().clone(), supports, s_scores, outputs_test, k=3)
            loss = loss_enp+0.1*loss_local

            optimizer.zero_grad()
            lr_scheduler.step_update((epoch * num_steps + idx) // config.TRAIN.ACCUMULATION_STEPS)
            loss.backward()
            optimizer.step()
            loss_meter.update(loss_local.item(), images.size(0))
            loss_entropy_meter.update(loss_enp.item(), images.size(0))
            ...
            prob = torch.nn.functional.softmax(outputs_test, dim=1).float()
            for bb in range(prob.shape[0]):
                # 收集输出和目标
                all_outputs.extend(prob[bb, :].unsqueeze(0).data.cpu().numpy())
                # targets = torch.nn.functional.one_hot(targets.squeeze(-1).long(), num_classes=2)
...
@torch.no_grad()
def obtain_label_org(loader, model, valdata=''):
    model.eval()
    start_test = True
    img_names=[]
    with torch.no_grad():
    ...
    # 分别合并 fea、output、label
    all_fea = merge_dict_list(all_fea)
    all_output = merge_dict_list(all_output)
    ...
    acc = np.sum(pred_label == final_label.float().numpy()) / len(all_fea)
    log_str = valdata+' Accuracy = {:.2f}% -> {:.2f}%'.format(accuracy * 100, acc * 100)
    print(log_str+'\n')
    model.train()
    return pred_label, img_names #, labelset
    ...
    for kk in range(len(lrs)):
        ...
        with open(file_path, 'a', encoding='utf-8') as file:
            re = f'{args.model_name} {config.BASE_LR}:\n' \
                 ...
                 f'ORG global_all_test_acc: {avg_test_acc_org} ---> TSD global_all_test_acc: {avg_test_acc_tent}\n' \
                 f'TSD global_shanxi_test_avg: {shanxi_test_shot[0]}, shanxi_test_auc: {shanxi_test_shot[-1]}, shanxi_test_acc: {shanxi_test_shot[1]}, shanxi_test_sensitivity: {shanxi_test_shot[2]}, shanxi_test_specificity: {shanxi_test_shot[3]}\n' \
                 f'TSD global_guizhou_test_avg: {guizhoutest_shot[0]}, guizhou_test_auc: {guizhoutest_shot[-1]}, guizhou_test_acc: {guizhoutest_shot[1]}, guizhou_test_sensitivity: {guizhoutest_shot[2]}, guizhou_test_specificity: {guizhoutest_shot[3]}\n\n'
