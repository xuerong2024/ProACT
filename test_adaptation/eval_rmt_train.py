...
def main(config, ptname):
    ...
    if os.path.isfile(ckp_path):
        ...
        model = train_one_epoch(config, model, data_loader_val, data_loader_test, val_index='shanxi_test')
        ...
        model= train_one_epoch(config, model, data_loader_val, data_loader_test2, val_index='guizhou_test')
...
def train_one_epoch(config, model, datta_loader_train, data_loader, val_index=''):
    ...
    for epoch in range(config.TRAIN.EPOCHS):
        ...
        # logger.info(
        #     f'lr: {args.base_lr} {valdata} {ptnames}' + ' Global_results Average: ' + "%.4f" % avg + ' Accuracy: ' + "%.4f" % acc + ' AUC.5: ' + "%.4f" % auc + ' Sensitivity: ' + "%.4f" % sensitivity + ' Specificity.5: ' + "%.4f" % specificity)
        validate_network(data_loader, model_ema, val_index)
    return model_ema
    ...
    for kk in range(len(lrs)):
        ...
        with open(file_path, 'a', encoding='utf-8') as file:
            re = f'{args.model_name} {config.BASE_LR}:\n' \
                 ...
                 f'RMT_TRAIN global_shanxi_test_avg: {shanxi_test_shot[0]}, shanxi_test_auc: {shanxi_test_shot[-1]}, shanxi_test_acc: {shanxi_test_shot[1]}, shanxi_test_sensitivity: {shanxi_test_shot[2]}, shanxi_test_specificity: {shanxi_test_shot[3]}\n' \
                 f'RMT_TRAIN global_guizhou_test_avg: {guizhoutest_shot[0]}, guizhou_test_auc: {guizhoutest_shot[-1]}, guizhou_test_acc: {guizhoutest_shot[1]}, guizhou_test_sensitivity: {guizhoutest_shot[2]}, guizhou_test_specificity: {guizhoutest_shot[3]}\n\n'
