'''ROID [WACV2024] Universal Test-time Adaptation through Weight Ensembling, Diversity Weighting,  and Prior Correction, WACV2024'''
import os
import time
import json
import random
...
def main(config, ptname):
    ...
    if os.path.isfile(ckp_path):
        ...
        model = train_one_epoch(config, model, data_loader_test, val_index='shanxi_test')
        shanxi_test_shot=validate_network(data_loader_test, model, val_index='shanxi_test', use_prior_correction=True)
        gc.collect()
        ...
        model = train_one_epoch(config, model, data_loader_test2, val_index='guizhou_test')
        guizhoutest_shot=validate_network(data_loader_test2, model, val_index='guizhou_test', use_prior_correction=True)
...
@torch.no_grad()
def validate_network(val_loader, model, val_index='shanxi_val', use_prior_correction=False):
    model.eval()
    header = 'Test:'
    # 初始化输出和目标列表
    all_outputs = []
    ...
    for i, (inp, mask, target, imgname) in enumerate(val_loader):
        ...
        if use_prior_correction:
            prior = output.softmax(1).mean(0)
            smooth = max(1 / output.shape[0], 1 / output.shape[1]) / torch.max(prior)
            smoothed_prior = (prior + smooth) / (1 + smooth * output.shape[1])
            output *= smoothed_prior
...
def train_one_epoch(config, model, data_loader, val_index='', temperature=1/3, momentum_probs=0.9):
    num_classes=2
    ...
    class_probs_ema = 1 / num_classes * torch.ones(num_classes).to(device)
    slr = SoftLikelihoodRatio()
    softmax_entropy = Entropy()  # not used as loss
    for epoch in range(config.TRAIN.EPOCHS):
        ...
        for idx, (images, masks, targetss, img_names) in enumerate(data_loader):
            ...
            with torch.no_grad():
                ...
                weights_div = 1 - F.cosine_similarity(class_probs_ema.unsqueeze(dim=0), outputs.softmax(1), dim=1)
                ...
                weights = torch.exp(weights_div * weights_cert / temperature)
                weights[mask] = 0.
                class_probs_ema = update_model_probs(x_ema=class_probs_ema, x=outputs.softmax(1).mean(0), momentum=momentum_probs)

            # calculate the soft likelihood ratio loss
            loss_out = slr(logits=outputs)
            loss_out = loss_out * weights
            loss_out = loss_out[~mask]
            loss_slr = loss_out.sum() / outputs.shape[0]

            outputs_aug,_ = model(tta_transforms(images[~mask]))
            loss_sys = (symmetric_cross_entropy(x=outputs_aug, x_ema=outputs[~mask]) * weights[
                        ~mask]).sum() / outputs.shape[0]
            loss=loss_slr+loss_sys
            loss.backward()
            optimizer.step()
            optimizer.zero_grad()
            ...
            loss_contrastive_meter.update(loss_sys.item(), images.size(0))
            loss_self_meter.update(loss_slr.item(), images.size(0))
        ...
        # logger.info(
        #     f'lr: {args.base_lr} {valdata} {ptnames}' + ' Global_results Average: ' + "%.4f" % avg + ' Accuracy: ' + "%.4f" % acc + ' AUC.5: ' + "%.4f" % auc + ' Sensitivity: ' + "%.4f" % sensitivity + ' Specificity.5: ' + "%.4f" % specificity)
        validate_network(data_loader, model_ema, val_index, use_prior_correction=True)
    return model_ema
    ...
    for kk in range(len(lrs)):
        ...
        with open(file_path, 'a', encoding='utf-8') as file:
            re = f'{args.model_name} {config.BASE_LR}:\n' \
                 ...
                 f'ORG global_all_test_acc: {avg_test_acc_org} ---> ROID global_all_test_acc: {avg_test_acc_tent}\n' \
                 f'ROID_train global_shanxi_test_avg: {shanxi_test_shot[0]}, shanxi_test_auc: {shanxi_test_shot[-1]}, shanxi_test_acc: {shanxi_test_shot[1]}, shanxi_test_sensitivity: {shanxi_test_shot[2]}, shanxi_test_specificity: {shanxi_test_shot[3]}\n' \
                 f'ROID_train global_guizhou_test_avg: {guizhoutest_shot[0]}, guizhou_test_auc: {guizhoutest_shot[-1]}, guizhou_test_acc: {guizhoutest_shot[1]}, guizhou_test_sensitivity: {guizhoutest_shot[2]}, guizhou_test_specificity: {guizhoutest_shot[3]}\n\n'
