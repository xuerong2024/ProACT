'''SAR [ICLR’23] Towards stable test-time adaptation in dynamic wild world'''
import os
import time
import json
import random
...
def train_one_epoch(config, model, data_loader, margin=0.8*math.log(2)):
    log_path = os.path.join(config.OUTPUT, 'logs')
    os.makedirs(os.path.dirname(log_path), exist_ok=True)
    params, param_names = collect_params(model)
    base_optimizer = torch.optim.AdamW
    optimizer = SAM(params, base_optimizer, eps=config.TRAIN.OPTIMIZER.EPS, betas=config.TRAIN.OPTIMIZER.BETAS,
    ...
    for epoch in range(config.TRAIN.EPOCHS):
        ...
        for idx, (images, masks, targetss, img_names) in enumerate(data_loader):
            ...
            entropys = softmax_entropy(outputs_test)
            margin = torch.quantile(entropys, q=0.4, dim=0, keepdim=False)
            filter_ids_1 = torch.where(entropys < margin)
            # 判断filter_ids_1是否为空（torch.where返回元组，第一个元素是索引张量，判断其长度是否为0）
            if len(filter_ids_1[0]) == 0:
                # 无可靠样本，跳过后续所有适配流程，直接进入下一轮迭代
                print(f"Batch {idx}: filter_ids_1 is empty, skip adaptation")
                gc.collect()
                del outputs_test, entropys
                # 4. 清理CUDA缓存（释放未使用的内存给系统）
                if torch.cuda.is_available():
                    torch.cuda.empty_cache()
                continue  # 跳过当前batch，进入下一个batch
            ...
            outputs_test2 = model(images)
            is_tuple = isinstance(outputs_test2, tuple)
            if is_tuple:
                outputs_test2 = outputs_test2[0]
            entropys2 = softmax_entropy(outputs_test2)
            entropys2 = entropys2[filter_ids_1]  # second time forward
            margin = torch.quantile(entropys2, q=0.4, dim=0, keepdim=False)
            ...
            if len(filter_ids_2[0]) == 0:
                # 二次筛选无可靠样本，跳过二次反向传播和optimizer.second_step
                print(f"Batch {idx}: filter_ids_2 is empty, skip second step update")
                # 关键：将参数从扰动后的\Theta+\hat{\epsilon}回退到原始\Theta（手动回退，等价于second_step的第一步）
                with torch.no_grad():
                ...
                optimizer.zero_grad()
                gc.collect()
                del outputs_test, outputs_test2, entropys, entropys2
                # 4. 清理CUDA缓存（释放未使用的内存给系统）
                if torch.cuda.is_available():
                    torch.cuda.empty_cache()
                continue  # 进入下一个batch
            ...
            loss_entropy_meter.update(loss_second.item(), images.size(0))
...
if __name__ == '__main__':
    ...
    for kk in range(len(lrs)):
        ...
        config.OUTPUT = '/disk3/wjr/workspace/sec_proj4/baseline_expeript/TTA/CHOOSEN_200/SLGMS/SAR/'
        folder_path = config.OUTPUT
        if not os.path.exists(folder_path):
            os.makedirs(folder_path)
        file_name = 'SAR_result.txt'
        ...
        with open(file_path, 'a', encoding='utf-8') as file:
            re = f'{args.model_name} {config.BASE_LR}:\n' \
                 ...
                 f'SAR_margin0.4 global_shanxi_test_avg: {shanxi_test_shot[0]}, shanxi_test_auc: {shanxi_test_shot[-1]}, shanxi_test_acc: {shanxi_test_shot[1]}, shanxi_test_sensitivity: {shanxi_test_shot[2]}, shanxi_test_specificity: {shanxi_test_shot[3]}\n' \
                 f'SAR_margin0.4 global_guizhou_test_avg: {guizhoutest_shot[0]}, guizhou_test_auc: {guizhoutest_shot[-1]}, guizhou_test_acc: {guizhoutest_shot[1]}, guizhou_test_sensitivity: {guizhoutest_shot[2]}, guizhou_test_specificity: {guizhoutest_shot[3]}\n\n'

            file.write(re)
# f'ORG global_all_test_acc: {avg_test_acc_org} ---> SAR global_all_test_acc: {avg_test_acc_tent}\n' \
