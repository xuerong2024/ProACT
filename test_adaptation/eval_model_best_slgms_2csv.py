...
def main(config, pt_path, logger_path):
    ...
    if os.path.isfile(ckp_path):
        ...
        state_dict = torch.load(save_path, map_location='cpu', weights_only=False)['model']
        msg = model.load_state_dict(state_dict, strict=False)
        shanxi_test_shot = validate_network_glb(data_loader_test, model, 'shanxi_test',
                                                save_path=logger_path + '/analyze/')
...
if __name__ == '__main__':
    ...
    for ii in range(len(radios)):
        for jj in range(len(num_prototypes)):
                ...
                config.OUTPUT = '/disk2/wxr/vi_feiqu_baseline/baseline_expeript/TTA/SLGMS/OUR/3e-05lr_15lpara_4prototypes_0.6rule_JSD/'
