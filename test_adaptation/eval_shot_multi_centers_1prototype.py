...
def train_one_epoch(config, model, data_loader_prototype, data_loader, dataloader_small):
    mem_label, imgnames = obtain_multi_centers_label(data_loader_prototype, dataloader_small, model, NUM_PROTO = config.NUM_PROTOTYPES)
    ...
    for epoch in range(config.TRAIN.EPOCHS):
        ...
        mem_label, imgnames = obtain_multi_centers_label(data_loader_prototype, data_loader, model, NUM_PROTO = config.NUM_PROTOTYPES, valdata = f'{config.NUM_PROTOTYPES}prototypes_2centers')
        mem_label = torch.from_numpy(mem_label).to(device)
    ...
    mem_label, imgnames = obtain_multi_centers_label(data_loader_prototype, data_loader, model, NUM_PROTO = config.NUM_PROTOTYPES)
    return model
...
@torch.no_grad()
def obtain_multi_centers_label(loader_prototype, loader, model, NUM_PROTO = 1, INIT_WEIGHT = 0.5, valdata = ''):
    ...
    dd = cdist(final_fea, initc_soft[labelset], 'cosine')
    pred_label = dd.argmin(axis=1)
    pred_label = labelset[pred_label]

    for round in range(1):
        pred_rel_refine = pred_label[reliable_mask_np]
        fea_rel_refine = final_fea[reliable_mask_np]

        aff = np.eye(K)[pred_rel_refine]  # (Nr, K)
        initc = aff.T @ fea_rel_refine  # (K, D)
        initc = initc / (1e-8 + aff.sum(axis=0)[:, None])
...
def obtain_multi_centers_multi_prototype_label(loader_prototype, loader, model, NUM_PROTO = 3, INIT_WEIGHT = 0.5, valdata = ''):
    ...
    # threshold
    tau = np.maximum(0,np.percentile(margin, 80))  # 或 25
    reliable_mask = margin > tau
