...
'''Fisher:自适应选择重要值大的参数进行更新：Adapting In-Domain Few-Shot Segmentation to New Domains without Source Domain Retraining'''
...
def compute_structure_fisher_scores(model, support_loader, device):
    """
    Compute Structure Fisher Scores for all convolutional layers in the model.
    ...
    # Step 2: Forward + backward on support set
    for i, (inp, mask, target, imgname) in enumerate(support_loader):
        inp = (inp * mask).to(device)
        target = target.to(device)
...
def main(config, pt_path):
    ...
    if os.path.isfile(ckp_path):
        ...
        model = train_one_epoch(config, model, data_loader_test, data_loader_test_small)
        ...
        for k, v in model.named_parameters():
            if k.__contains__("head"):
                v.requires_grad = False
        model = train_one_epoch(config, model, data_loader_test2, data_loader_test2_small)
...
def train_one_epoch(config, model, data_loader, dataloader_small):
    # mem_label, imgnames = obtain_label(dataloader_small, model)
    ...
    #
    #     if k.__contains__("downsample_layers.0"):
    #         v.requires_grad = True
    #     else:
    #         v.requires_grad = False
    ...
    for epoch in range(config.TRAIN.EPOCHS):
        ...
        for idx, (images, masks, targetss, img_names) in enumerate(data_loader):
            ...
            threshold = find_quantile(fisher_flat, config.RADIO/100.)
            optimizer.step()
            optimizer.zero_grad()
            # Teacher update, see line 12 in Algorithm 2 in the main paper
            model_ema = update_ema_variables(ema_model=model_ema, model=model, alpha_teacher=0.999)
                    ...
                    for npp, p in m.named_parameters():
                        if npp in ['weight', 'bias'] and p.requires_grad:
                            mask_fish = (fisher_dict[
                                             f"{nm}.{npp}"] < threshold).float().to(device)  # masking makes it restore candidate
                            mask = mask_fish
                            with torch.no_grad():
    ...
    mem_label = obtain_label(data_loader, model_ema)
    return model_ema
...
if __name__ == '__main__':
    ...
    for kk in range(len(lrs)):
        for jj in range(len(radios)):
            ...
            file_name = 'shot_perbatch_fisher_result.txt'
            ...
            with open(file_path, 'a', encoding='utf-8') as file:
                re = f'{args.model_name} {config.BASE_LR}:\n' \
                     ...
                     f'SHOT TRAIN_FEA_fisher_pesudo_{radio} global_shanxi_test_avg: {shanxi_test_shot[0]}, shanxi_test_auc: {shanxi_test_shot[-1]}, shanxi_test_acc: {shanxi_test_shot[1]}, shanxi_test_sensitivity: {shanxi_test_shot[2]}, shanxi_test_specificity: {shanxi_test_shot[3]}\n' \
                     f'SHOT TRAIN_FEA_fisher_pesudo_{radio} global_guizhou_test_avg: {guizhoutest_shot[0]}, guizhou_test_auc: {guizhoutest_shot[-1]}, guizhou_test_acc: {guizhoutest_shot[1]}, guizhou_test_sensitivity: {guizhoutest_shot[2]}, guizhou_test_specificity: {guizhoutest_shot[3]}\n\n'
