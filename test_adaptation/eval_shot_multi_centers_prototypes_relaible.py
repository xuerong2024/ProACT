...
def train_one_epoch(config, model, data_loader_prototype, data_loader, dataloader_small, num_classes=2):
    ...
    for epoch in range(config.TRAIN.EPOCHS):
        ...
        for idx, (images, masks, targetss, img_names) in enumerate(data_loader):
            ...
            reliable_mask_ii = reliable_mask[
                idx * config.DATA.BATCH_SIZE:idx * config.DATA.BATCH_SIZE + images.shape[0]]
            ...
            one_hot = F.one_hot(pred[~reliable_mask_ii], num_classes).float()
            soft_label = (1 - eps) * one_hot + eps / num_classes

            # 计算对数概率
            log_prob = F.log_softmax(outputs_test[~reliable_mask_ii], dim=1)

            # 计算平滑损失

            if np.any(reliable_mask_ii):
                classifier_loss = nn.CrossEntropyLoss()(outputs_test[reliable_mask_ii, ...], pred[reliable_mask_ii, ...])
            else:
                classifier_loss=torch.tensor(0).to(device)
            unreliable_mask = ~reliable_mask_ii
            if np.any(unreliable_mask):
                smooth_loss = -(soft_label * log_prob).sum(dim=1).mean()
                softmax_out = nn.Softmax(dim=1)(outputs_test[~reliable_mask_ii, ...])
                entropy_loss = torch.mean(Entropy(softmax_out))
            else:
                smooth_loss = torch.tensor(0).to(device)
                entropy_loss = torch.tensor(0).to(device)
            loss = classifier_loss + smooth_loss
            ...
            loss_smooth_meter.update(smooth_loss.item(), images.size(0))
            ...
            if idx % config.PRINT_FREQ == 0:
                ...
                logger.info(
                    ...
                    f'classifier_smooth_loss {loss_smooth_meter.val:.4f} ({loss_smooth_meter.avg:.4f})\t'
                    f'entropy_loss {loss_entropy_meter.val:.4f} ({loss_entropy_meter.avg:.4f})\t'
                )
    # mem_label = obtain_label(data_loader, model)
    mem_label, reliable_mask, imgnames = obtain_multi_centers_multi_prototype_label(data_loader_prototype, data_loader, model, NUM_PROTO = config.NUM_PROTOTYPES)
    return model
...
if __name__ == '__main__':
    ...
    num_prototypes=[4]
    smooth_eps=[0,0.1]
    for pp in range(len(smooth_eps)):
        for kk in range(len(lrs)):
            for ii in range(len(radios)):
                for jj in range(len(num_prototypes)):
                    ...
                    config.SMOOTH_EPS = smooth_eps[pp]
                    ...
                    file_name = 'shot_biaozhun_reliable_multi_prototypes_result.txt'
                    ...
                    with open(file_path, 'a', encoding='utf-8') as file:
                        re = f'{args.model_name} {config.BASE_LR} RADIO:{config.RADIO} NUM_PROTOTYPES:{config.NUM_PROTOTYPES}:\n' \
                             ...
                             f'SHOT smooth_{config.SMOOTH_EPS}_TRAIN_FEA_multi_prototype_{config.NUM_PROTOTYPES} global_shanxi_test_avg: {shanxi_test_shot[0]}, shanxi_test_auc: {shanxi_test_shot[-1]}, shanxi_test_acc: {shanxi_test_shot[1]}, shanxi_test_sensitivity: {shanxi_test_shot[2]}, shanxi_test_specificity: {shanxi_test_shot[3]}\n' \
                             f'SHOT smooth_{config.SMOOTH_EPS}_TRAIN_FEA_multi_prototype_{config.NUM_PROTOTYPES} global_guizhou_test_avg: {guizhoutest_shot[0]}, guizhou_test_auc: {guizhoutest_shot[-1]}, guizhou_test_acc: {guizhoutest_shot[1]}, guizhou_test_sensitivity: {guizhoutest_shot[2]}, guizhou_test_specificity: {guizhoutest_shot[3]}\n\n'
