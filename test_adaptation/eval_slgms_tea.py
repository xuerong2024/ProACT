'''TEA: Test-Time Energy Adaptation, CVPR2024 '''
import os
import time
import json
import random
...
class EnergyModel(nn.Module):
    ...
    def forward(self, x, y=None):
        ...
        if y is None:
            return logits, logits.logsumexp(1), logits
        else:
...
def train_one_epoch(config, model, data_loader, buffer_size=10000, sgld_steps=10, sgld_lr=1, sgld_std=0.01,
    ...
    for epoch in range(config.TRAIN.EPOCHS):
        ...
        for idx, (images, masks, targetss, img_names) in enumerate(data_loader):
            ...
            # loss = torch.clamp(-(energy_real - energy_fake), min=0.0)
            loss.backward()
            optimizer.step()
            optimizer.zero_grad()

            # loss_meter.update(classifier_loss.item(), images.size(0))
            loss_entropy_meter.update(loss.item(), images.size(0))
...
if __name__ == '__main__':
    ...
    for kk in range(len(lrs)):
        ...
        config.OUTPUT = '/disk3/wjr/workspace/sec_proj4/baseline_expeript/TTA/CHOOSEN_200/SLGMS/TEA_3/'
        folder_path = config.OUTPUT
        if not os.path.exists(folder_path):
            os.makedirs(folder_path)
        file_name = 'TEA_result.txt'
        ...
        with open(file_path, 'a', encoding='utf-8') as file:
            re = f'{args.model_name} {config.BASE_LR}:\n' \
                 ...
                 f'TEA global_shanxi_test_avg: {shanxi_test_shot[0]}, shanxi_test_auc: {shanxi_test_shot[-1]}, shanxi_test_acc: {shanxi_test_shot[1]}, shanxi_test_sensitivity: {shanxi_test_shot[2]}, shanxi_test_specificity: {shanxi_test_shot[3]}\n' \
                 f'TEA global_guizhou_test_avg: {guizhoutest_shot[0]}, guizhou_test_auc: {guizhoutest_shot[-1]}, guizhou_test_acc: {guizhoutest_shot[1]}, guizhou_test_sensitivity: {guizhoutest_shot[2]}, guizhou_test_specificity: {guizhoutest_shot[3]}\n\n'

            file.write(re)

# f'ORG global_all_test_acc: {avg_test_acc_org} ---> TEA global_all_test_acc: {avg_test_acc_tent}\n' \
