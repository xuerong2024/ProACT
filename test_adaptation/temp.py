...
# 添加自定义路径到全局模块搜索路径中
sys.path.append('/disk2/wxr/vi_feiqu_baseline/proj4_feiqu_baseline//')
...
def train_one_epoch(config, model, data_loader):
    ...
    for epoch in range(config.TRAIN.EPOCHS):
        ...
        for idx, (images, masks, targetss, img_names) in enumerate(data_loader):
            ...
            entropy_loss = softmax_entropy(outputs_test).mean(0)
            # entropy_loss = dirichlet_entropy(outputs_test).mean(0)
            loss = entropy_loss
...
if __name__ == '__main__':
    ...
    for kk in range(len(lrs)):
        ...
        config.OUTPUT = '/disk2/wxr/vi_feiqu_baseline/baseline_expeript/TTA/CALGM/temp2/TENT/'
        ...
        # file_name = 'COME_result.txt'
        file_name = 'TENT_result.txt'
        folder_path = config.OUTPUT
        if not os.path.exists(folder_path):
            os.makedirs(folder_path)
        file_path = os.path.join(folder_path, file_name)
        logger_path = os.path.join(config.OUTPUT,
                                   f'{config.BASE_LR}lr_{config.DATA.BATCH_SIZE}batch_{config.TRAIN.EPOCHS}epoch')
        ...
        with open(file_path, 'a', encoding='utf-8') as file:
            re = f'{args.model_name} {config.BASE_LR} {config.TRAIN.EPOCHS}epoch:\n' \
                 ...
                 f'ORG global_all_test_acc: {avg_test_acc_org} ---> TENT global_all_test_acc: {avg_test_acc_tent}\n' \
                 f'total_inference_time: {total_inference_time} seconds\n'\
                 f'TENT global_shanxi_test_avg: {shanxi_test_shot[0]}, shanxi_test_auc: {shanxi_test_shot[-1]}, shanxi_test_acc: {shanxi_test_shot[1]}, shanxi_test_sensitivity: {shanxi_test_shot[2]}, shanxi_test_specificity: {shanxi_test_shot[3]}\n' \
                 f'TENT global_guizhou_test_avg: {guizhoutest_shot[0]}, guizhou_test_auc: {guizhoutest_shot[-1]}, guizhou_test_acc: {guizhoutest_shot[1]}, guizhou_test_sensitivity: {guizhoutest_shot[2]}, guizhou_test_specificity: {guizhoutest_shot[3]}\n\n'
