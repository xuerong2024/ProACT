...
def main(config, pt_path, logger_path):
    ...
    if os.path.isfile(ckp_path):
        ...
        shanxi_test_shot = train_one_epoch(config, model, data_loader_test, val_index='shanxi_test', save_path=logger_path+'/analyze/')
        ...
        guizhoutest_shot = train_one_epoch(config, model, data_loader_test2, val_index='guizhou_test', save_path=logger_path+'/analyze/')
...
def train_one_epoch(config, model, data_loader, val_index='', temperature=1/3, momentum_probs=0.9, save_path=''):
    num_classes=2
    ...
    for epoch in range(5):
        ...
        for idx, (images, masks, targetss, img_names) in enumerate(data_loader):
            ...
            optimizer.step()
            optimizer.zero_grad()
            model = ema_update_model(
                model_to_update=model,
                model_to_merge=src_model,
                momentum=0.99,
                device=device
            )
...
if __name__ == '__main__':
    ...
    for kk in range(len(lrs)):
        ...
        # config.OUTPUT = '/disk3/wjr/workspace/sec_proj4/baseline_expeript/TTA/CHOOSEN_200/CALGM/ROID/'
        config.OUTPUT = '/disk3/wjr/workspace/sec_proj4/baseline_expeript/TTA/CALGM/ROID/epoch5/'
        ...
        with open(file_path, 'a', encoding='utf-8') as file:
            re = f'{args.model_name} {config.BASE_LR}:\n' \
                 ...
                 f'ROID global_shanxi_test_avg: {shanxi_test_shot[0]}, shanxi_test_auc: {shanxi_test_shot[-1]}, shanxi_test_acc: {shanxi_test_shot[1]}, shanxi_test_sensitivity: {shanxi_test_shot[2]}, shanxi_test_specificity: {shanxi_test_shot[3]}\n' \
                 f'ROID global_guizhou_test_avg: {guizhoutest_shot[0]}, guizhou_test_auc: {guizhoutest_shot[-1]}, guizhou_test_acc: {guizhoutest_shot[1]}, guizhou_test_sensitivity: {guizhoutest_shot[2]}, guizhou_test_specificity: {guizhoutest_shot[3]}\n\n'
