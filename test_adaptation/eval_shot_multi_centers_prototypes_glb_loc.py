    ...
    for ii in range(len(lrs)):
        for jj in range(len(num_prototypes)):
            for kk in range(len(LOCRADIOS)):
                ...
                config.OUTPUT='/disk3/wjr/workspace/sec_proj4/baseline_expeript/TTA/CHOOSEN_200/CALGM/OUR/'
                ...
                logger_path = os.path.join(config.OUTPUT,
                                         f'{config.DATA.BATCH_SIZE}batch_{config.BASE_LR}lr_{config.RADIO}lpara_{config.NUM_PROTOTYPES}prototypes_{config.LOCRADIO}rule_JSD')
