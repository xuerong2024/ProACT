...
def main(config, pt_path, logger_path):
    ...
    if os.path.isfile(ckp_path):
        ...
        # state_dict = torch.load('/disk3/wjr/workspace/sec_proj4/baseline_expeript/TTA/CALGM/ablation/reliable/pred_kmeans/15batch_8e-05lr_15lpara_4prototypes_0.6rule_JSD/shanxi_test.pth', map_location='cpu', weights_only=False)['model']
        # msg = model.load_state_dict(state_dict, strict=False)
        # print(msg)
...
def train_one_epoch(config, model, data_loader_prototype, data_loader, dataloader_small):
    ...
    mem_label, imgnames = obtain_multi_centers_multi_prototype_label1(data_loader_prototype, dataloader_small, model, NUM_PROTO = config.NUM_PROTOTYPES)
    ...
    for epoch in range(config.TRAIN.EPOCHS):
        ...
        mem_label, imgnames = obtain_multi_centers_multi_prototype_label1(data_loader_prototype, data_loader, model, NUM_PROTO = config.NUM_PROTOTYPES, valdata = f'{config.NUM_PROTOTYPES}prototypes_2centers')
        mem_label = torch.from_numpy(mem_label).to(device)
    ...
    mem_label, imgnames = obtain_multi_centers_multi_prototype_label1(data_loader_prototype, data_loader, model, NUM_PROTO = config.NUM_PROTOTYPES)
    return model
...
@torch.no_grad()
def obtain_multi_centers_multi_prototype_label1(loader_prototype, loader, model, NUM_PROTO = 3, INIT_WEIGHT = 0.5, valdata = ''):
    #NUM_PROTO = 3  # 每个类别的 prototype 数（2~3 推荐）
    #INIT_WEIGHT = 0.5 softmax-weighted center 与 kmeans center 融合比例
    model.eval()
    start_test = True
    ...
    # threshold
    reliable_mask = np.zeros_like(predict.cpu().numpy(), dtype=bool)

    # ---- class 0 ----
    if mask0.sum() > 0:
        m0 = all_output[mask0, 0]
        tau0 = np.maximum(0, np.percentile(m0, 20))  # 30 比 20 更稳
        reliable_mask[mask0] = m0 > tau0

    # ---- class 1 ----
    if mask1.sum() > 0:
        m1 = all_output[mask1, 1]
        tau1 = np.maximum(0, np.percentile(m1, 20))
        reliable_mask[mask1] = m1 > tau1
...
@torch.no_grad()
def obtain_multi_centers_multi_prototype_label0(loader_prototype, loader, model, NUM_PROTO = 3, INIT_WEIGHT = 0.5, valdata = ''):
    #NUM_PROTO = 3  # 每个类别的 prototype 数（2~3 推荐）
    #INIT_WEIGHT = 0.5 softmax-weighted center 与 kmeans center 融合比例
    model.eval()
    start_test = True
    ...
    for ii in range(len(radios)):
        for jj in range(len(num_prototypes)):
            for kk in range(len(LOCRADIOS)):
                ...
                config.OUTPUT='/disk3/wjr/workspace/sec_proj4/baseline_expeript/TTA/CALGM/ablation/reliable/pred_kmeans'
