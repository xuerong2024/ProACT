...
def train_one_epoch(config, model, data_loader_prototype, data_loader, dataloader_small):
    ...
    # 转为列表以便随机采样
    layer_items = list(layer_param_count.items())  # [(layer_key, numel), ...]

    total_params = sum(layer_param_count.values())
    target_params = total_params * 0.15

    # 2. 随机打乱层顺序
    random.shuffle(layer_items)

    # 3. 贪心选取，直到累计 ≥ 15%
    cumulative = 0
    trainable_layer_set = set()
    for layer_key, numel in layer_items:
        cumulative += numel
    ...
    for name, param in model.named_parameters():
        layer_key = '.'.join(name.split('.')[:-1])
        param.requires_grad = (layer_key in trainable_layer_set)
    ...
    for ii in range(len(radios)):
        for jj in range(len(num_prototypes)):
            for kk in range(len(LOCRADIOS)):
                ...
                config.OUTPUT='/disk3/wjr/workspace/sec_proj4/baseline_expeript/TTA/CALGM/ablation/para_trainable/random15_trainable'
