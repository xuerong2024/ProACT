...
def train_one_epoch(config, model, data_loader_prototype, data_loader, dataloader_small):
    ...
    mem_label, imgnames = obtain_multi_centers_multi_prototype_label0(data_loader_prototype, dataloader_small, model, NUM_PROTO = config.NUM_PROTOTYPES)
    ...
    for epoch in range(config.TRAIN.EPOCHS):
        ...
        mem_label, imgnames = obtain_multi_centers_multi_prototype_label0(data_loader_prototype, data_loader, model, NUM_PROTO = config.NUM_PROTOTYPES, valdata = f'{config.NUM_PROTOTYPES}prototypes_2centers')
        mem_label = torch.from_numpy(mem_label).to(device)
    ...
    mem_label, imgnames = obtain_multi_centers_multi_prototype_label0(data_loader_prototype, data_loader, model, NUM_PROTO = config.NUM_PROTOTYPES)
    return model
    ...
    for ii in range(len(radios)):
        for jj in range(len(num_prototypes)):
            for kk in range(len(LOCRADIOS)):
                ...
                config.OUTPUT='/disk3/wjr/workspace/sec_proj4/baseline_expeript/TTA/CALGM/ablation/reliable/allsampleskmeans'
