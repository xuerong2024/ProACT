...
def obtain_multi_centers_multi_prototype_label(loader_prototype, loader, model, NUM_PROTO = 3, INIT_WEIGHT = 0.5, valdata = ''):
    ...
    if mask0.sum() > 0:
        m0 = margin[mask0]
        tau0 = np.maximum(0, np.percentile(m0, 100-config.PERC))  # 30 比 20 更稳
        reliable_mask[mask0] = m0 > tau0

    # ---- class 1 ----
    if mask1.sum() > 0:
        m1 = margin[mask1]
        tau1 = np.maximum(0, np.percentile(m1, 100-config.PERC))
    ...
    for ii in range(len(radios)):
        for jj in range(len(num_prototypes)):
            for kk in range(len(PERCS)):
                ...
                config.OUTPUT='/disk3/wjr/workspace/sec_proj4/baseline_expeript/TTA/ablation/reliable/PERC_HYPE'
                ...
                logger_path = os.path.join(config.OUTPUT,
                                         f'15batch_{config.BASE_LR}lr_{config.RADIO}lpara_{config.PERC}PREC_{config.NUM_PROTOTYPES}prototypes_{config.LOCRADIO}rule_JSD')
                ...
                with open(file_path, 'a', encoding='utf-8') as file:
                    re = f"{args.model_name} {config.BASE_LR} learnable_para_RADIO:{config.RADIO} reliable_PREC: {config.PERC} NUM_PROTOTYPES:{config.NUM_PROTOTYPES}: JSDnew_sum_rule_weight:{config.LOCRADIO}\n"
                        # --- Shanxi ORG ---
                    re += "=== Shanxi Test (ORG) ===\n"
                    # global
                    g = shanxi_test_org[0]
