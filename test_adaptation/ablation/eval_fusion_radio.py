...
def train_one_epoch(config, model, data_loader_prototype, data_loader, dataloader_small):
    ...
    mem_label, reliable_mask,imgnames = obtain_multi_centers_multi_prototype_label(data_loader_prototype, dataloader_small, model, NUM_PROTO = config.NUM_PROTOTYPES, INIT_WEIGHT=config.FRADIOS)
    ...
    for epoch in range(config.TRAIN.EPOCHS):
        ...
        mem_label, reliable_mask, imgnames = obtain_multi_centers_multi_prototype_label(data_loader_prototype, data_loader, model, NUM_PROTO = config.NUM_PROTOTYPES, valdata = f'{config.NUM_PROTOTYPES}prototypes_2centers', INIT_WEIGHT=config.FRADIOS)
        mem_label = torch.from_numpy(mem_label).to(device)
    ...
    for ii in range(len(radios)):
        for jj in range(len(num_prototypes)):
            for kk in range(len(FRADIOS)):
                ...
                config.OUTPUT='/disk3/wjr/workspace/sec_proj4/baseline_expeript/TTA/CALGM/ablation/reliable/FUSION_radio_batch5'
                ...
                with open(file_path, 'a', encoding='utf-8') as file:
                    re = f"{args.model_name} {config.BASE_LR} prototype_fusion_radio:{config.FRADIOS} learnable_para_RADIO:{config.RADIO} NUM_PROTOTYPES:{config.NUM_PROTOTYPES}: JSDnew_sum_rule_weight:{config.LOCRADIO}\n"
                        # --- Shanxi ORG ---
                    re += "=== Shanxi Test (ORG) ===\n"
                    # global
                    g = shanxi_test_org[0]
