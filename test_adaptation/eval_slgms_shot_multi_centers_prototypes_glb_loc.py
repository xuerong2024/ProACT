...
def build_phe_loader(args):
    ...
    dataset_val = Shanxi_w7masks_5Subregions_wsubroi_Dataset(args.data_root + 'seg_rec_mask_img_1024',
    ...
    # dataset_test = Shanxi_w7masks_5Subregions_wsubroi_Dataset(args.data_root + 'seg_rec_mask_img_1024',
    #                                                         args.data_root + 'seg_rec_mask_1024',
    ...
    #                                                         sub_img_size=256, )
    #
    # dataset_test2 = Shanxi_w7masks_5Subregions_wsubroi_Dataset('/disk3/wjr/dataset/nejm/guizhoudataset/seg_rec_mask_img_1024','/disk3/wjr/dataset/nejm/guizhoudataset/seg_rec_mask_1024',
    ...
    #                                                           data_subregions_transform=global_transfo2_subregions,
    #                                                           sub_img_size=256, )
    dataset_test3 = Shanxi_w7masks_5Subregions_wsubroi_Dataset(args.data_root + 'seg_rec_mask_img_1024',
                                                            args.data_root + 'seg_rec_mask_1024',
                                                            txtpath=args.data_root + 'chinese_biaozhunwo_1.txt',
                                                            csvpath=args.data_root + 'subregions_label_shanxi_all.xlsx',
                                                            data_transform=global_transfo2,
    ...
    dataset_test2 = Shanxi_w7masks_5Subregions_wsubroi_Dataset(
        '/disk3/wjr/dataset/nejm/guizhoudataset/seg_rec_mask_img_1024',
...
def main(config, pt_path, logger_path):
    ...
    if os.path.isfile(ckp_path):
        ...
        ckp_path2 = os.path.join(logger_path, 'shanxi_test.pth')
...
def validate_network_glb(val_loader, model, valdata='shanxi_val', save_path=''):
    ...
    for idx, (images, masks, labels, sub_rois, masks_index, imgname) in enumerate(val_loader):
        ...
        subregions_outputs, _ = model(subregions_imgs)
        ...
        for bb in range(prob.shape[0]):
            ...
            result["Global Pred Labels"] = pred_global_labels
            ...
            if aa == 0:
                ...
                pred_global_labels = 'Health'
            result["Local Pred Labels"] = pred_global_labels

            aa = torch.argmax((global_prob[bb, aa] + prob[bb, aa]) / 2)
            if aa == 0:
                pred_global_labels = 'Sick'
            else:
                pred_global_labels = 'Health'
            result["Global_Local Pred Labels"] = pred_global_labels
    ...
    return [avg, acc, sensitivity, specificity, auc], [avg_local, acc_local, sensitivity_local, specificity_local,
                                                       auc_local], [avg_local_global, acc_local_global,
                                                                    sensitivity_local_global, specificity_local_global,
                                                                    auc_local_global],
...
def compute_structure_fisher_scores_pesudo(model, support_loader, mem_label, device):
    ...
    for name, param in model.named_parameters():
        if ('patch_embed' in name or 'layers' in name) and param.requires_grad:
...
def train_one_epoch(config, model, data_loader_prototype, data_loader, dataloader_small):
    ...
    for epoch in range(config.TRAIN.EPOCHS):
        ...
        for idx, (images, masks, labels, sub_rois, masks_index, imgname) in enumerate(data_loader):
            ...
            subregions_outputs, sub_feas = model(subregions_imgs)
            subregions_outputs = torch.nn.functional.softmax(subregions_outputs, dim=1)
            subregions_outputs2 = subregions_outputs.view(samples.shape[0], 6, -1)
    ...
    for ii in range(len(lrs)):
        for jj in range(len(num_prototypes)):
            for kk in range(len(LOCRADIOS)):
                ...
                config.BASE_LR = lrs[ii]
                radio = radios[0]
                config.RADIO=radio
                num_prototype=num_prototypes[jj]
                config.NUM_PROTOTYPES=num_prototype
                config.LOCRADIO=LOCRADIOS[kk]
                config.OUTPUT='/disk3/wjr/workspace/sec_proj4/baseline_expeript/TTA/CHOOSEN_200/SLGMS/OUR/'
                ...
                logger_path = os.path.join(config.OUTPUT,
                                         f'{config.BASE_LR}lr_{config.RADIO}lpara_{config.NUM_PROTOTYPES}prototypes_{config.LOCRADIO}rule_JSD')
