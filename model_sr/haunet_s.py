...
class HAUNet(nn.Module):
    ...
    def __init__(self, up_scale=4, img_channel=3, width=96,enc_middle_blk_num=4,dec_middle_blk_num=1, enc_blk_nums=[4,4], dec_blk_nums=[4,4], heads = [1,2,4],):
        ...
        self.intro = nn.Conv2d(in_channels=img_channel, out_channels=width, kernel_size=3, padding=1, stride=2,
                               groups=1,
                               bias=True)
        ...
        self.up = nn.Sequential(
            nn.Conv2d(in_channels=width, out_channels=img_channel *(2* up_scale) ** 2, kernel_size=3, padding=1, stride=1,
                      groups=1, bias=True),
            nn.PixelShuffle(up_scale* 2)
        )
        self.up_scale = up_scale

    def forward(self, inp):
        inp_lr=F.interpolate(inp, scale_factor=1/self.up_scale, mode='bilinear')
        inp_hr = F.interpolate(inp_lr, scale_factor=self.up_scale, mode='bilinear')
        x = self.intro(inp_lr)
...
if __name__ == '__main__':
    import time
    net = HAUNet(up_scale=4, width=64, enc_blk_nums=[2, 2, 1],dec_blk_nums=[1,1, 1],enc_middle_blk_num=1,dec_middle_blk_num=1, heads = [1,2,4,8]).cuda()
