...
def parse_option():
    ...
    parser.add_argument('--output', default='/disk3/wjr/workspace/sec_proj4/baseline_expeript/feiqu5cls_2global/224/contra_20251127/', type=str, metavar='PATH',
...
def build_phe_loader(args):
    ...
    dataset_train = Shanxi_wmask_feiqu_Dataset(args.data_root + '/subregionsimgs',
                                               args.data_root + '/subregionsmasks',
    ...
    dataset_train2 = Shanxi_wmask_feiqu_Dataset(args.data_root + '/subregionsimgs',
                                                args.data_root + '/subregionsmasks',
                                                txtpath=args.data_root + 'stage1_health_txt/selected_dcm_saomiao_900_health308_sick_592/train_subregions.txt',
                                                data_transform=transform,
                                                )
    dataset_val = Shanxi_wmask_feiqu_Dataset(args.data_root + '/subregionsimgs',
                                             args.data_root + '/subregionsmasks',
                                             txtpath=args.data_root + 'stage1_health_txt/selected_dcm_saomiao_900_health308_sick_592/val_subregions.txt',
                                             data_transform=transform,
                                             )

    dataset_test = Shanxi_wmask_feiqu_Dataset('/disk3/wjr/dataset/nejm/guizhoudataset/subregionsimgs',
                                              '/disk3/wjr/dataset/nejm/guizhoudataset/subregionsmasks',
    ...
    return data_loader_train, data_loader_train2, data_loader_val, data_loader_test, data_loader_val_global, data_loader_test_global, data_loader_test2_global
...
from loss.ordinal_loss import *
def main(config, ptname='model_ema_best_auc_shanxi_val'):
    data_loader_train, data_loader_train2, data_loader_val, data_loader_test, data_loader_val_global,data_loader_test_global, data_loader_test2_global = build_phe_loader(args)
    ...
    if args.model_name=='resnet50':
        ...
        # model = convnexttiny_org_pooling(pretrained=True, drop_path_rate=0.)
        # model.head.fc = nn.Linear(768, 5)
    ...
    # for epoch in range(config.TRAIN.START_EPOCH, config.TRAIN.EPOCHS):
        ...
        config, data_loader_val, model,
        threshold=0.5, valdata='shanxi_val')
        avg_val_acc=np.mean(shanxi_val_peracc[~np.isnan(shanxi_val_peracc)])
        if shanxi_val_accuracy > max_auc_ema_avg3:
            best_epoch = epoch
            max_auc_ema_avg3 = max(max_auc_ema_avg3, shanxi_val_accuracy)
            save_test_checkpoint(config, epoch, model, max_auc_ema_avg3, optimizer, lr_scheduler,
                             loss_scaler,
                             logger, ptname='model_best_localacc_shanxi_val.pth')
        else:
            max_auc_ema_avg3 = max(max_auc_ema_avg3, shanxi_val_accuracy)
        if shanxi_val_auc > max_acc_ema_avg2:
            best_epoch = epoch
            max_acc_ema_avg2 = max(max_acc_ema_avg2, shanxi_val_auc)
            save_test_checkpoint(config, epoch, model, max_acc_ema_avg2, optimizer, lr_scheduler,
                             loss_scaler,
                             logger, ptname='model_best_localauc_shanxi_val.pth')
        else:
            max_acc_ema_avg2 = max(max_acc_ema_avg2, shanxi_val_auc)

        loss, auc, acc, writer = validate_global(data_loader_val_global, model,epoch, writer, index='shanxi_val')
        test_loss, test_auc,test_acc, writer = validate_global(data_loader_test_global, model, epoch, writer, index='shanxi_test')
        # test_loss, test_auc_ema, test_acc_ema, writer = validate(data_loader_test, model_ema.ema, epoch, writer, index='shanxi_test_ema')
        loss_test2, auc_test2, test_acc2,writer = validate_global(data_loader_test2_global, model, epoch, writer, index='guizhou_test')
        ...
        if avg_val_acc > max_acc_avg:
            best_epoch = epoch
            max_acc_avg = max(max_acc_avg, avg_val_acc)
            save_test_checkpoint(config, epoch, model, max_acc_avg, optimizer, lr_scheduler,
                                 loss_scaler,
                                 logger, ptname='model_best_avglocalacc_shanxi_val.pth')
        else:
            max_acc_avg = max(max_acc_avg, avg_val_acc)
...
def train_one_epoch(config, model, criterion, data_loader, optimizer, epoch, mixup_fn, lr_scheduler, loss_scaler, model_ema=None, writer=None):
    ...
    for idx, (images, masks, labels, imgnames) in enumerate(data_loader):
        ...
        output, feas= model(samples)

        loss_cls = criterion(output, targets)
        rank_loss=model.margin_criterion(feas, targets.squeeze(-1).long(),
                                                  step=epoch * num_steps + idx)
        loss=loss_cls+rank_loss
        ...
        if idx % config.PRINT_FREQ == 0:
            ...
            writer.add_scalar('Shanxi_train cls_loss', loss_cls.item(), epoch * num_steps + idx)
            writer.add_scalar('Shanxi_train rank_loss', rank_loss.item(), epoch * num_steps + idx)
...
@torch.no_grad()
def validate_global(data_loader, model,epoch, writer, index='shanxi_val'):
    model.eval()
    ...
    acc_local, auc_local, sensitivity_local, specificity_local, f1_local, precision_local, recall_local, threshold_mcc = adaptive_calculate_metrics_global(
        all_outputs, all_targets, threshold=0.5)

    avg_local = (acc_local + auc_local + specificity_local + sensitivity_local) / 4
    writer.add_scalar(index + ' auc', auc_local, epoch)
    writer.add_scalar(index + ' acc', acc_local, epoch)
    writer.add_scalar(index + ' specificity', specificity_local, epoch)
    writer.add_scalar(index + ' sensitivity', sensitivity_local, epoch)

    # writer.add_scalar(index + ' threshold', threshold, epoch)
    logger.info(
        f'{index} epoch: {epoch}' + ' auc: ' + "%.4f" % auc_local + ' acc: ' + "%.4f" % acc_local)

    return loss_meter.avg, auc_local, acc_local, writer
...
def validate_final_global(config, data_loader, model,threshold=None, valdata='shanxi_test'):
    ...
    logger.info(f'{valdata} Local2_results Average: {avg_local:.4f}, Accuracy: {acc_local:.4f}, AUC: {auc_local:.4f}, Sensitivity: {sensitivity_local:.4f}, Specificity: {specificity_local:.4f}')
...
def validate_final(config, data_loader, model,threshold=None, valdata='shanxi_test'):
    ...
    for idx, (images, masks, targets, _) in enumerate(data_loader):
        ...
        # compute output
        # with torch.cuda.amp.autocast(enabled=config.AMP_ENABLE):
        subregions_outputs,_ = model(images)
    ...
    # 计算整体指标
    # 计算整体指标
    accuracy, auc, per_acc, per_auc = adaptive_calculate_metrics(all_outputs, all_targets, valdata)
    # 打印结果
    print(f'Accuracy: {accuracy:.4f}, AUC: {auc:.4f}', 'per_acc:', per_acc, 'per_auc:',per_auc)
    # logger.info(f'Accuracy: {accuracy:.4f}, AUC: {auc:.4f}', 'per_acc:', per_acc, 'per_auc:',per_auc)
    logger.info(f'Accuracy: {accuracy:.4f}, AUC: {auc:.4f}, per_acc: {per_acc}, per_auc: {per_auc}')
...
if __name__ == '__main__':
    ...
    aug_gaussian=[True]
    loss_type=['ce']
    # pt_names=['model_ema_best_acc_shanxi_val']
    pt_names=['model_best_localauc_shanxi_val', 'model_best_localacc_shanxi_val', 'model_best_avglocalacc_shanxi_val','model_best_acc_shanxi_test','model_best_acc_guizhou_test']
        ...
        for qq in range(len(aug_gaussian)):
            for pp in range(len(aug_rot)):
                for jj in range(len(model_name)):
                    ...
                    config.OUTPUT = os.path.join(args.output, 'wmask_subcls_nodropout', args.model_name+'_allsick_health_weighted_cloc_rank2',
