...
def main(config, ptname='model_ema_best_auc_shanxi_val'):
    ...
    shanxi_val_accuracy, shanxi_val_auc, shanxi_val_peracc, shanxi_val_perauc,shanxi_threshold = validate_final(
        config, data_loader_val, model,
        valdata='shanxi_val', threshold=shanxi_train_threshold)

    guizhou_test_accuracy, guizhou_test_auc, guizhou_test_peracc, guizhou_test_perauc,guizhou_threshold = validate_final(
        config, data_loader_test, model,
        valdata='guizhou_test', threshold=shanxi_train_threshold)
...
def validate_final(config, data_loader, model,valdata='shanxi_test', threshold=None):
    ...
    # 计算整体指标
    # 计算整体指标
    accuracy, auc, per_acc, per_auc, threshold = adaptive_calculate_metrics(all_outputs, all_targets, valdata, threshold)
    # 打印结果
