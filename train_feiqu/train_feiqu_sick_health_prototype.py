...
def build_phe_loader(args):
    ...
    dataset_train_prototype = Shanxi_wmask_feiqu_Dataset(args.data_root + '/subregionsimgs',
                                                         args.data_root + '/subregionsmasks',
                                                         txtpath=args.data_root + 'chinese_ilo_biaozhun_subregions.txt',
    ...
    dataset_train2 = Shanxi_Submasks_Subregions_Dataset(args.data_root + '/subregionsimgsbig', args.data_root + '/subregionsmasksbig',
    ...
    dataset_val = Shanxi_Submasks_Subregions_Dataset(args.data_root + '/subregionsimgsbig', args.data_root + '/subregionsmasksbig',
                                                              txtpath=args.data_root + 'stage1_health_txt/selected_dcm_saomiao_900_health308_sick_592/val.txt',
                                                              data_subregions_transform=global_transfo_subregions,
                                                              )
    dataset_test = Shanxi_Submasks_Subregions_Dataset(args.data_root + '/subregionsimgsbig', args.data_root + '/subregionsmasksbig',
                                                              txtpath=args.data_root + 'stage1_health_txt/selected_dcm_saomiao_900_health308_sick_592/test.txt',
                                                              data_subregions_transform=global_transfo_subregions,
                                                              )
    dataset_test2 = Shanxi_Submasks_Subregions_Dataset('/disk3/wjr/dataset/nejm/guizhoudataset/subregionsimgsbig', '/disk3/wjr/dataset/nejm/guizhoudataset/subregionsmasksbig',
    ...
    return data_loader_train, data_loader_train_prototype, data_loader_train2, data_loader_val, data_loader_test, data_loader_test2
...
def main(config, ptname='model_ema_best_auc_shanxi_val'):
    data_loader_train, data_loader_train_prototype, data_loader_train2, data_loader_val, data_loader_test, data_loader_test2 = build_phe_loader(args)
    ...
    if os.path.isfile(ckp_path):
        ...
        shanxi_train_accuracy, shanxi_train_auc, shanxi_train_sens, shanxi_train_spec = validate_final(
            config, data_loader_val, model,
            threshold=0.5, valdata='shanxi_val')

        shanxi_val_accuracy, shanxi_val_auc, shanxi_val_sens, shanxi_val_spec = validate_final(
            config, data_loader_test, model,
            threshold=0.5, valdata='shanxi_test')

        guizhou_test_accuracy, guizhou_test_auc, guizhou_test_sens, guizhou_test_spec = validate_final(
            config, data_loader_test2, model,
            threshold=0.5, valdata='guizhou_test')

        return shanxi_train_auc, shanxi_train_accuracy, shanxi_train_spec, shanxi_train_sens, shanxi_val_auc, shanxi_val_accuracy, shanxi_val_spec, shanxi_val_sens, guizhou_test_auc, guizhou_test_accuracy, guizhou_test_spec, guizhou_test_sens
    ...
    # for epoch in range(config.TRAIN.START_EPOCH, config.TRAIN.EPOCHS):
        ...
        loss, auc, acc, writer = validate(data_loader_val, model,epoch, writer, index='shanxi_val')
...
def validate(data_loader, model,epoch, writer, index='shanxi_val'):
    ...
    for idx, (images, masks, targets, _) in enumerate(data_loader):
        images = (images * masks).to(device).squeeze(0)
...
def validate_final(config, data_loader, model,threshold=None, valdata='shanxi_test'):
    ...
    # 计算整体指标
    # 计算整体指标
    acc_local, auc_local, sensitivity_local, specificity_local, f1_local, precision_local, recall_local, threshold_mcc = adaptive_calculate_metrics(all_outputs, all_targets)
        ...
        for qq in range(len(aug_gaussian)):
            for pp in range(len(aug_rot)):
                for jj in range(len(model_name)):
                    ...
                    config.OUTPUT = os.path.join(args.output, 'wmask_subcls_nodropout',args.model_name+'_databig_sick_health',
                    ...
                    for pt_name in pt_names:
                        ...
                        with open(file_path, 'a', encoding='utf-8') as file:
                            re = f'{args.model_name} {args.base_lr}: {pt_name}:\n' \
                                 f'shanxi_train_auc: {shanxi_train_auc}, shanxi_train_acc: {shanxi_train_acc}, shanxi_train_spec: {shanxi_train_perauc}, shanxi_train_sens: {shanxi_train_peracc}\n' \
                                 f'shanxi_val_auc: {shanxi_val_auc}, shanxi_val_acc: {shanxi_val_acc}, shanxi_val_spec: {shanxi_val_perauc}, shanxi_val_sens: {shanxi_val_peracc}\n' \
                                 f'guizhou_test_auc: {guizhou_test_auc}, guizhou_test_acc: {guizhou_test_accuracy}, guizhou_test_spec: {guizhou_test_perauc}, guizhou_test_sens: {guizhou_test_peracc}\n\n'
                            file.write(re)
