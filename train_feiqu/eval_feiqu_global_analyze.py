...
def build_phe_loader(args):
    ...
    dataset_train_global = Shanxi_Submasks_Subregions_Dataset5(args.data_root + '/subregionsimgsbig',
                                                              ...
                                                              subtxtpath=args.data_root + 'stage1_health_txt/selected_dcm_saomiao_900_health308_sick_592/train_subregions.txt',
                                                              data_subregions_transform=global_transfo_subregions,
                                                              )
    dataset_test_global = Shanxi_Submasks_Subregions_Dataset5(args.data_root + '/subregionsimgsbig',
                                                             args.data_root + '/subregionsmasksbig',
                                                             txtpath=args.data_root + 'stage1_health_txt/selected_dcm_saomiao_900_health308_sick_592/val.txt',
                                                             subtxtpath=args.data_root + 'stage1_health_txt/selected_dcm_saomiao_900_health308_sick_592/val_subregions.txt',
                                                             data_subregions_transform=global_transfo_subregions,
                                                             )
    dataset_test2_global = Shanxi_Submasks_Subregions_Dataset5('/disk3/wjr/dataset/nejm/guizhoudataset/subregionsimgsbig',
                                                      '/disk3/wjr/dataset/nejm/guizhoudataset/subregionsmasksbig',
                                                      txtpath='/disk3/wjr/dataset/nejm/guizhoudataset/guizhou_one.txt',
                                                      subtxtpath='/disk3/wjr/dataset/nejm/guizhoudataset/guizhou_one_subregions.txt',
                                                      data_subregions_transform=global_transfo_subregions,
                                                      )
    ...
    #                                                    txtpath='/disk3/wjr/dataset/nejm/guizhoudataset/guizhou_one.txt',
    #                                                    data_subregions_transform=global_transfo_subregions)
    data_loader_train_global = torch.utils.data.DataLoader(
    ...
    return data_loader_train2, data_loader_val, data_loader_test, data_loader_train_global, data_loader_test_global, data_loader_test2_global
...
def main(config, ptname='model_ema_best_auc_shanxi_val'):
    data_loader_train2, data_loader_val, data_loader_test, data_loader_train_global, data_loader_test_global, data_loader_test2_global = build_phe_loader(args)
    ...
    if os.path.isfile(ckp_path):
        checkpoint = torch.load(ckp_path, map_location="cpu", weights_only=False)
        model.load_state_dict(checkpoint['model'], strict=True)
    # validate_final(config, data_loader_train2, model, valdata='shanxi_train')
    ...
    validate_final_global(data_loader_train_global, model, valdata='shanxi_train')
    validate_final_global(data_loader_test_global, model, valdata='shanxi_val')
    validate_final_global(data_loader_test2_global, model, valdata='guizhou_test')
    #
...
def validate_final_global(data_loader, model, valdata='shanxi_test'):
    ...
    all_outputs = []
    all_targets = []
    all_outputs_sub = defaultdict(list)
    all_targets_sub = defaultdict(list)
    regions = ['left_top', 'right_top', 'left_center', 'right_center', 'left_bottom', 'right_bottom']
    data_results = []
    for idx, (images, masks, sub_targets, targets, imgnames) in enumerate(data_loader):
        result = {}
        result["Image Index"] = imgnames[0]
        result["Finding Labels"] = imgnames[0].split('_')[0]
        ...
        # subregions_outputs2, subregions_outputs_sick, subregions_outputs_health, subregions_fea = model(images)
        # subregions_outputs2 = nn.Softmax(dim=-1)(subregions_outputs2)
        # outputs_sick3_softmax = nn.Softmax(dim=-1)(subregions_outputs_sick) * subregions_outputs2[:,
        #                                                                       1].unsqueeze(-1)
        # outputs_health2_softmax = nn.Softmax(dim=-1)(subregions_outputs_health) * subregions_outputs2[:,
        #                                                                           0].unsqueeze(-1)
        # prob = torch.cat((outputs_health2_softmax, outputs_sick3_softmax), dim=-1)
        # subregions_outputs2=subregions_outputs2.reshape(masks.shape[0], masks.shape[1],2)
        # prob=prob.reshape(masks.shape[0], masks.shape[1],5)
        # global_prob = local2global_prob(subregions_outputs2)
        # global_pred = int(torch.argmax(global_prob).data.cpu().numpy())
        ...
        if global_pred == 1:
            ...
            global_pred = 'Sick'
        result["pred"] = global_pred
        for jj in range(len(regions)):
            region=regions[jj]
            for ii in range(subregions_outputs2.shape[0]):
                labels_01 = sub_targets[ii, jj].unsqueeze(0).clone()
                local_pred=int(labels_01.squeeze().data.cpu().numpy())
                if local_pred==0:
                    local_pred='0_0'
                elif local_pred==1:
                    local_pred='0_1'
                    ...
                    local_pred = '1+'
                result[region + "_label"]=local_pred
                labels_01[labels_01 == 1] = 0
                labels_01[labels_01 > 1] = 1
                all_outputs_sub[region].extend(subregions_outputs2[ii, jj].unsqueeze(0).float().data.cpu().numpy())
                all_targets_sub[region].extend(labels_01.data.cpu().numpy())
                region_pred=int(torch.argmax(prob[ii, jj]).data.cpu().numpy())
                if region_pred==0:
                    region_pred='0_0'
                elif region_pred==1:
                    region_pred='0_1'
                    ...
                    region_pred = '1+'
                result[region + "_pred"] = region_pred
    ...
    df_data_results.to_csv(config.OUTPUT + '/' + valdata+ '_local_global_result.csv',
                                 index=False)
    # 合并所有输出和目标
    ...
    for region in regions:
        all_outputs_sub[region]= np.array(all_outputs_sub[region])
        all_targets_sub[region]= np.array(all_targets_sub[region])
...
if __name__ == '__main__':
    ...
    aug_gaussian=[True]
    loss_type=['ce']
    pt_names=['model_best_acc_guizhou_test']
        ...
        for qq in range(len(aug_gaussian)):
            for pp in range(len(aug_rot)):
                for jj in range(len(model_name)):
                    ...
                    # config.OUTPUT = '/disk3/wjr/workspace/sec_proj4/baseline_expeript/feiqu5cls_2global/224/contra_20251127/wmask_subcls_nodropout/calgm_databig_allsick_health/224_batch60_lr0.0001_testall'
                    config.OUTPUT = '/disk3/wjr/workspace/sec_proj4/baseline_expeript/feiqu5cls_2global/224/contra_20251127/wmask_subcls_nodropout/convnext_databig_allsick_health_addativepooling/224_batch60_lr5e-05_testall'
