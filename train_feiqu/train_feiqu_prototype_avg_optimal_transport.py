        ...
        for region in pos_cache:
            ...
            for class_index in sorted(pos_cache[region].keys()):
                ...
                for item in pos_cache[region][class_index]:
                    cache_names.append(item[-1])
                    cache_keys.append(item[0])
                    cache_values.append(class_index)
        cache_keys = torch.cat(cache_keys, dim=0).permute(1, 0, 2).to(device)
            ...
            for ij, item in enumerate(image_features):
                ...
                for posii in pos:
                    if posii in imgnames_prototype[ij]:
                        name = posii
                update_cache(pos_cache[name], int(targets[ij]), [item.unsqueeze(0), loss[ij].unsqueeze(0)], pos_shot_capacity, imgname=imgnames_prototype[ij])
    return pos_cache
...
def Entropy(input_):
    ...
    return entropy
def update_cache(cache, pred, features_loss, shot_capacity, include_prob_map=False, imgname=''):
    """Update cache with new features and loss, maintaining the maximum shot capacity."""
    with torch.no_grad():
        # item = features_loss if not include_prob_map else features_loss[:2] + [features_loss[2]]
        # 安全地构造 item：始终把 imgname 附加到最后
        if include_prob_map and len(features_loss) >= 3:
            item = features_loss[:2] + [features_loss[2], imgname]
        else:
            # 即使 include_prob_map=True，但没提供 prob_map，也降级处理
            item = features_loss[:2] + [imgname]  # 只取前两个 + name
        if pred in cache:
...
import numpy as np
import torch
#分布外（OOD）样本往往导致 softmax 输出更均匀 → 熵更高。
...
if __name__ == '__main__':
    ...
    aug_gaussian=[True]
    loss_type=['ce']
    pt_names=['ftda_model_best_auc_shanxi_train', 'ftda_model_best_acc_shanxi_train','ftda_model_best_acc_shanxi_test','ftda_model_best_acc_guizhou_test']
        ...
        for qq in range(len(aug_gaussian)):
            for pp in range(len(aug_rot)):
                for jj in range(len(model_name)):
                    ...
                    config.OUTPUT = os.path.join(args.output, 'wmask_subcls_nodropout',args.model_name+'_databig_allsick_health_prototype_avg_OPTIMAL_TRANSPORT',
