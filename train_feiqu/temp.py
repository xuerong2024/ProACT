...
def build_phe_loader(args):
    ...
    dataset_val_global = Shanxi_Submasks_Subregions_Dataset(args.data_root + '/subregionsimgs',
                                                            args.data_root + '/subregionsmasks',
    ...
    dataset_test_global = Shanxi_Submasks_Subregions_Dataset(args.data_root + '/subregionsimgs',
                                                             args.data_root + '/subregionsmasks',
    ...
    dataset_test2_global = Shanxi_Submasks_Subregions_Dataset(
        '/disk3/wjr/dataset/nejm/guizhoudataset/subregionsimgs',
        '/disk3/wjr/dataset/nejm/guizhoudataset/subregionsmasks',
...
from loss.ordinal_loss import WeightedKappaLoss
...
def train_one_epoch(config, model, criterion, data_loader, optimizer, epoch, mixup_fn, lr_scheduler, loss_scaler, model_ema=None, writer=None):
    ...
    for idx, (images, masks, labels, imgnames) in enumerate(data_loader):
        ...
        rank_loss=loss_fn(output, targets)
...
if __name__ == '__main__':
    ...
    pt_names=['model_best_localauc_shanxi_val', 'model_best_localacc_shanxi_val', 'model_best_acc_shanxi_train','model_best_auc_shanxi_train','model_best_acc_shanxi_test','model_best_acc_guizhou_test']
        ...
        for qq in range(len(aug_gaussian)):
            for pp in range(len(aug_rot)):
                for jj in range(len(model_name)):
                    ...
                    config.OUTPUT = os.path.join(args.output, 'wmask_subcls_nodropout', args.model_name+'_allsick_health_weighted_kappa_rank',
