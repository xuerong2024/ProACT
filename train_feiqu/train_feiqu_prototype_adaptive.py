...
def build_phe_loader(args):
    ...
    if args.aug_gaussian == True:
        transform_list.append(GaussianBlur(0.2))
    transform_list.extend([
        transforms.ToTensor(),
...
def main(config, ptname):
    ...
    if os.path.isfile(ckp_path):
        ...
        pos_cache = prototype_cache(model, data_loader_prototype)
        cache_keys = []
        cache_values = []
        ...
        cache_targets = torch.tensor(cache_values, dtype=torch.long)
        cache_values = F.one_hot(cache_targets, num_classes=5).float().to(device).detach()
        num_classes = 5  # 0/0 to 3/+
        class_counts = torch.bincount(cache_targets, minlength=num_classes).float()
        class_freq = class_counts / class_counts.sum()
        # 3. 计算平衡权重 (1/sqrt(freq))
        base_weights = 1.0 / torch.sqrt(class_freq + 1e-8)  # [11]
        base_weights = base_weights / base_weights.mean()  # 归一化使平均权重=1
        cache_values_weighted = cache_values * base_weights.view(1, -1).to(device)  # [N, 11]
        shanxi_train_accuracy, shanxi_train_auc, shanxi_train_peracc, shanxi_train_perauc = validate_final(
            data_loader_train2, model, cache_keys=cache_keys, cache_values=cache_values_weighted, valdata='shanxi_train')

        shanxi_val_accuracy, shanxi_val_auc, shanxi_val_peracc, shanxi_val_perauc = validate_final(
            data_loader_val, model, cache_keys=cache_keys, cache_values=cache_values_weighted, valdata='shanxi_val')

        guizhou_test_accuracy, guizhou_test_auc, guizhou_test_peracc, guizhou_test_perauc = validate_final(
            data_loader_test, model, cache_keys=cache_keys, cache_values=cache_values_weighted, valdata='guizhou_test')
        ...
        shanxi_val_accuracy_global, shanxi_val_auc_global, shanxi_val_sens_global, shanxi_val_spec_global = validate_final_global2(
            ...
            valdata='shanxi_test', cache_keys=cache_keys, cache_values=cache_values_weighted,
            )
        guizhou_test_accuracy_global, guizhou_test_auc_global, guizhou_test_sens_global, guizhou_test_spec_global = validate_final_global2(
            ...
            pos_alpha=2, pos_beta=5,
            valdata='guizhou_test', cache_keys=cache_keys, cache_values=cache_values_weighted,
            )
    ...
    for epoch in range(config.TRAIN.START_EPOCH, 30):
        ...
        writer = train_one_epoch(config, model, criterion, data_loader_train, optimizer, epoch, lr_scheduler,
                                 cache_keys,cache_values_weighted, pos_alpha=2, pos_beta=5, writer=writer)
        ...
        auc, acc, writer = validate_final_global(data_loader_val_global, model, pos_alpha=2, pos_beta=5,
                                                 valdata='shanxi_val', cache_keys=cache_keys, cache_values=cache_values_weighted, writer=writer,
                                                 epoch=epoch)
...
def train_one_epoch(config, model, criterion, data_loader, optimizer, epoch,lr_scheduler,cache_keys, cache_values, pos_alpha=2, pos_beta=5, writer=None):
    ...
    for idx, (images, masks, labels, imgnames) in enumerate(data_loader):
        ...
        if epoch>-1:
            ...
            tip_logits = _clsout
        loss_org = criterion(_clsout, targets)
        loss_tip = criterion(tip_logits, targets)
        loss=loss_org+loss_tip
        optimizer.zero_grad()
        ...
        if idx % config.PRINT_FREQ == 0:
            ...
            writer.add_scalar('Shanxitrain loss org classification', loss_org.item(), epoch * num_steps + idx)
            writer.add_scalar('Shanxitrain loss prototype calssification', loss_tip.item(), epoch * num_steps + idx)
...
def prototype_cache(model, data_loader_train_prototype):
    ...
    with torch.no_grad():
        ...
        for idx_prototype, (images_prototype, masks_prototype, labels_prototype, imgnames_prototype) in enumerate(
            ...
            for ij, item in enumerate(image_features):
                update_cache(pos_cache[str(int(targets[ij]))], int(targets[ij]), [item.unsqueeze(0)])
    return pos_cache
...
def Entropy(input_):
    ...
    return entropy
def update_cache(cache, pred, features_loss, include_prob_map=False):
...
if __name__ == '__main__':
    ...
    aug_gaussian=[True]
    loss_type=['ce']
    pt_names=['ftda_model_best_acc_shanxi_train','ftda_model_best_auc_shanxi_train','ftda_model_best_acc_shanxi_test','ftda_model_best_acc_guizhou_test']
        ...
        for qq in range(len(aug_gaussian)):
            for pp in range(len(aug_rot)):
                for jj in range(len(model_name)):
                    ...
                    config.OUTPUT = os.path.join(args.output, 'wmask_subcls_nodropout',args.model_name+'_databig_allsick_health_prototype_adaptive',
