...
def build_phe_loader(args):
    ...
    dataset_prototype = Shanxi_wmask_feiqu_Dataset(args.data_root + '/subregionsimgsbig',
                                             args.data_root + '/subregionsmasksbig',
                                             txtpath=args.data_root + 'chinese_ilo_book_subregions0.txt',
                                             data_transform=transform,
                                             )
...
def main(config, ptname):
    ...
    if args.model_name=='resnet50':
        ...
        model.prototype=nn.Parameter(torch.zeros(768, 180))
        print(msg)
    ...
    if os.path.isfile(ckp_path):
        ...
        # cache_keys= torch.cat((cache_keys,model.prototype), dim=-1).to(device)
        # cache_values= torch.cat((cache_values,cache_values), dim=0).to(device)
...
def train_one_epoch(config, model, criterion, data_loader, optimizer, epoch,lr_scheduler,cache_keys, cache_values, pos_alpha=2, pos_beta=5, writer=None):
    ...
    for idx, (images, masks, labels, imgnames) in enumerate(data_loader):
        ...
        if epoch>-1:
            ...
            affinity2 = image_features @ model.prototype
            # affinity = model.adapter(image_features)
            cache_logits2 = ((-1) * (pos_beta - pos_beta * affinity2)).exp() @ cache_values
            tip_logits = cache_logits + cache_logits2 + _clsout * pos_alpha
        ...
        for qq in range(len(aug_gaussian)):
            for pp in range(len(aug_rot)):
                for jj in range(len(model_name)):
                    ...
                    config.OUTPUT = os.path.join(args.output, 'wmask_subcls_nodropout',args.model_name+'_databig_allsick_health_prototype_two',
