...
def train_one_epoch(config, model, criterion, data_loader, optimizer, epoch, mixup_fn, lr_scheduler, loss_scaler, model_ema=None, writer=None):
    ...
    for idx, (images, masks, labels, imgnames) in enumerate(data_loader):
        ...
        subregions_outputs_0_1, subregions_outputs_sick, subregions_outputs_health, subregions_fea = model(samples)
        subregions_labels_0_1 = targets.squeeze(-1).clone()
        subregions_labels_0_1[(subregions_labels_0_1 == 0) | (subregions_labels_0_1 == 1)] = 0
        subregions_labels_0_1[(subregions_labels_0_1 == 2) | (subregions_labels_0_1 == 3) | (subregions_labels_0_1 == 4)] = 1
        ...
        outputs_health2_softmax = nn.Softmax(dim=-1)(subregions_outputs_health) * subregions_outputs_0_1_softmax[:,
                                                                                  0].unsqueeze(-1)
        subregions_outputs = torch.cat((outputs_health2_softmax, outputs_sick3_softmax), dim=-1)
        log_output = torch.log(subregions_outputs + 1e-8)  # 对softmax结果取log
        # 真实标签用类别索引形式，如 [1, 2]
        loss_subregions_5cls = nn.functional.nll_loss(log_output, targets.squeeze(-1).long())
        sub_feas = model.neck(subregions_fea).flatten(start_dim=1, end_dim=-1)
        pred_classes = torch.argmax(subregions_outputs, dim=1)  # shape: [batch, 1]
        ...
        sub_rank_clsloss = model.margin_criterion(sub_feas,
                                                  ...
                                                  step=epoch * num_steps + idx)
        loss=loss_subregions_5cls+loss_subregions_2cls+sub_rank_clsloss
        ...
        if idx % config.PRINT_FREQ == 0:
            ...
            writer.add_scalar('Shanxitrain 2clsloss', loss_subregions_2cls.item(), epoch * num_steps + idx)
            writer.add_scalar('Shanxitrain 5clsloss', loss_subregions_5cls.item(), epoch * num_steps + idx)
            writer.add_scalar('Shanxitrain rankloss', sub_rank_clsloss.item(), epoch * num_steps + idx)
...
def validate_global(data_loader, model,epoch, writer, index='shanxi_val'):
    ...
    for idx, (images, masks, targets, _) in enumerate(data_loader):
        ...
        subregions_outputs_0_1, subregions_outputs_sick, subregions_outputs_health, subregions_fea = model(images)
...
if __name__ == '__main__':
    ...
    aug_gaussian=[True]
    loss_type=['ce']
    pt_names=['model_best_acc_shanxi_train','model_best_acc_shanxi_test','model_best_acc_guizhou_test']
        ...
        for qq in range(len(aug_gaussian)):
            for pp in range(len(aug_rot)):
                for jj in range(len(model_name)):
                    ...
                    config.OUTPUT = os.path.join(args.output, 'wmask_subcls_nodropout',args.model_name+'_databig_allsick_health_new',
