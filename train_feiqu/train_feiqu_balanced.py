...
def build_phe_loader(args):
    ...
    dataset_train = Shanxi_wmask_feiqu_Dataset(args.data_root + '/subregionsimgsbig', args.data_root + '/subregionsmasksbig',
                                   txtpath=args.data_root + 'stage1_health_txt/selected_dcm_saomiao_900_health308_sick_592/train_sick_biaozhun_subregions_balanced_by_region_combo_weighted_yinying.txt',
    ...
    dataset_train2 = Shanxi_wmask_feiqu_Dataset(args.data_root + '/subregionsimgsbig', args.data_root + '/subregionsmasksbig',
                                               txtpath=args.data_root + 'stage1_health_txt/selected_dcm_saomiao_900_health308_sick_592/train_sick_biaozhun_subregions.txt',
                                               data_transform=transform,
                                               )
    dataset_val = Shanxi_wmask_feiqu_Dataset(args.data_root + '/subregionsimgsbig', args.data_root + '/subregionsmasksbig',
                                               txtpath=args.data_root + 'stage1_health_txt/selected_dcm_saomiao_900_health308_sick_592/val_sick_subregions.txt',
                                               data_transform=transform,
                                               )
...
def main(config, ptname='model_ema_best_auc_shanxi_val'):
    ...
    for epoch in range(config.TRAIN.START_EPOCH, config.TRAIN.EPOCHS):
        ...
        test_loss, test_auc,test_acc, writer = validate(data_loader_val, model, epoch, writer, index='shanxi_test')
        # test_loss, test_auc_ema, test_acc_ema, writer = validate(data_loader_test, model_ema.ema, epoch, writer, index='shanxi_test_ema')
        loss_test2, auc_test2, test_acc2,writer = validate(data_loader_test, model, epoch, writer, index='guizhou_test')
...
if __name__ == '__main__':
    ...
    model_name = ['convnext','swintiny','dinov3',]
    aug_rot=[True]
    aug_gaussian=[True]
    loss_type=['ce']
        ...
        for qq in range(len(aug_gaussian)):
            for pp in range(len(aug_rot)):
                for jj in range(len(model_name)):
                    ...
                    config.OUTPUT = os.path.join(args.output, 'wmask_subcls_nodropout',args.model_name+'_dtabig_balanced_detail',
