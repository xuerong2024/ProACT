...
def build_phe_loader(args):
    ...
    return data_loader_train2, data_loader_prototype, data_loader_val, data_loader_test, data_loader_test_global, data_loader_test2_global
...
def main(config, ptname='model_ema_best_auc_shanxi_val'):
    data_loader_train2, data_loader_prototype, data_loader_val, data_loader_test, data_loader_test_global, data_loader_test2_global = build_phe_loader(args)
    ...
    cache_keys = torch.cat(cache_keys, dim=0).permute(1, 0, 2).to(device).detach()
    ...
    # validate_final(config, data_loader_val, model, valdata='shanxi_val')
    # validate_final(config, data_loader_test, model, valdata='guizhou_test')
...
def prototype_cache(model, data_loader_train_prototype, pos_shot_capacity=6):
    ...
    with torch.no_grad():
        ...
        for idx_prototype, (images_prototype, masks_prototype, labels_prototype, imgnames_prototype) in enumerate(
            ...
            image_features=image_features.flatten(start_dim=-2, end_dim=-1).permute(0, 2,1)
...
def optimal_transport(image_features, cache_keys, logit_scale, max_iter=100):
    # 1. proto_features: 原型图像的局部特征 [N, p, d]，其中 N 是每个原型的局部区域数，p 是原型数量，d 是特征维度。
    # 2. img_features: 输入图像的局部特征 [M, b, d]，其中 M 是局部区域数，b 是批次大小，d 是特征维度。
    sim = torch.einsum('mbd,npd->mnbp', image_features, cache_keys).contiguous().flatten(start_dim=2)
    B=image_features.shape[1]
    P=cache_keys.shape[1]
    [M, N, BP] = sim.shape
    sim = sim.permute(2, 0, 1)
    wdist = 1.0 - sim
    u = torch.zeros(BP, M, dtype=sim.dtype, device=sim.device).fill_(1. / M)
    v = torch.zeros(BP, N, dtype=sim.dtype, device=sim.device).fill_(1. / N)
    eps = 0.1
    ...
    sim_op = sim_op.contiguous().view(B, P)
...
def validate_final(data_loader, model, cache_keys, cache_values, pos_alpha=2, pos_beta=5, valdata='shanxi_test'):
    ...
    all_org_outputs = defaultdict(list)
    # all_outputs = []
    # all_org_outputs = []
    # all_targets = []
    ...
    for idx, (images, masks, targets, imagename) in enumerate(data_loader):
        ...
        image_features = image_features.flatten(start_dim=-2, end_dim=-1).permute(2, 0, 1)
        image_features = image_features / image_features.norm(dim=-1, keepdim=True)

        # sim = torch.einsum('mbd, nbd -> bmn', image_features, cache_keys)
        affinity = optimal_transport(image_features, cache_keys, model.logit_scale, max_iter=100)
        cache_logits = ((-1) * (pos_beta - pos_beta * affinity)).exp() @ cache_values
        tip_logits = cache_logits + _clsout * pos_alpha
        # affinity_normalized = (affinity - affinity.mean()) / (affinity.std() + 1e-8)  # 标准化
        ...
        for ij in range(targets.shape[0]):
            ...
            all_org_outputs[name].extend(org_outputs[ij, ...].unsqueeze(0).float().data.cpu().numpy())
    ...
    for region in regions:
        ...
        all_org_outputs_r = np.array(all_org_outputs[region])
        # 计算整体指标
        # 计算整体指标
        accuracy, auc, per_acc, per_auc, threshold = adaptive_calculate_metrics(all_org_outputs_r, all_targets_r, valdata
                                                                                )
        print(f'ORG {region}: Accuracy: {accuracy:.4f}, AUC: {auc:.4f}', 'per_acc:', per_acc, 'per_auc:', per_auc)

        # 计算整体指标
        # 计算整体指标
        accuracy, auc, per_acc, per_auc, threshold = adaptive_calculate_metrics(all_outputs_r, all_targets_r, valdata)
        # 打印结果
        print(f'TDA {region}: Accuracy: {accuracy:.4f}, AUC: {auc:.4f}', 'per_acc:', per_acc, 'per_auc:', per_auc)
    ...
    # # 计算整体指标
    # # 计算整体指标
    # accuracy, auc, per_acc, per_auc, _ = adaptive_calculate_metrics(all_org_outputs, all_targets, valdata+'_org')
    ...
    # accuracy, auc, per_acc, per_auc, _ = adaptive_calculate_metrics(all_outputs, all_targets, valdata+'_tda')
    # # 打印结果
    # # print(f'{valdata} Accuracy: {accuracy:.4f}, AUC: {auc:.4f}', 'per_acc:', per_acc, 'per_auc:',per_auc)
...
if __name__ == '__main__':
    ...
    aug_gaussian=[True]
    loss_type=['ce']
    pt_names=['ftda_model_best_auc_shanxi_train']
        ...
        for qq in range(len(aug_gaussian)):
            for pp in range(len(aug_rot)):
                for jj in range(len(model_name)):
                    ...
                    config.OUTPUT = '/disk3/wjr/workspace/sec_proj4/baseline_expeript/feiqu5cls_2global/224/contra_20251127/wmask_subcls_nodropout/convnext_databig_allsick_health_prototype_avg_OPTIMAL_TRANSPORT/224_batch60_lr0.0001_testall'
