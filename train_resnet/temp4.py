...
def build_phe_loader(args):
    ...
    dataset_train = Shanxi_w7masks_Dataset(args.data_root + 'seg_rec_img_1024',
                                   ...
                                   csvpath=args.data_root + 'subregions_label_shanxi_ilo_new.xlsx',
...
from model_cls.dinov3.vit_dinov3 import vit_small, vit_base
...
def main(config, ptname='model_ema_best_auc_shanxi_val'):
    ...
    if args.model_name=='resnet50':
        ...
        model = get_model('vit-base-p32_in21k-pre_3rdparty_in1k-384px', pretrained=pretrained_cfg, backbone=dict(drop_path_rate=0.))
    ...
    for epoch in range(config.TRAIN.START_EPOCH, config.TRAIN.EPOCHS):
        ...
        if acc_ema > max_acc_ema_avg2:
            best_epoch = epoch
            max_acc_ema_avg2 = max(max_acc_ema_avg2, acc_ema)
            save_test_checkpoint(config, epoch, model_ema.ema, max_acc_ema_avg2, optimizer, lr_scheduler,
                                 loss_scaler,
...
def train_one_epoch(config, model, criterion, data_loader, optimizer, epoch, mixup_fn, lr_scheduler, loss_scaler, model_ema=None, writer=None):
    ...
    for idx, (images,masks, labels,_) in enumerate(data_loader):
        ...
        for kk in range(len(masks)):
            masks[kk]=masks[kk].to(device)
            labels[kk]=labels[kk].to(device)
...
if __name__ == '__main__':
    ...
    aug_gaussian=[True]
    loss_type=['ce']
    pt_names=['model_best_acc_shanxi_val','model_best_auc_shanxi_val','model_best_acc_shanxi_test','model_best_acc_guizhou_test', 'model_ema_best_acc_shanxi_test', 'model_ema_best_acc_guizhou_test', 'model_ema_best_acc_shanxi_val']
        ...
        for qq in range(len(aug_gaussian)):
            for pp in range(len(aug_rot)):
                for jj in range(len(model_name)):
                    ...
                    config.OUTPUT = os.path.join(args.output, 'wmask_nosubcls_nodropout2',
                                                 args.model_name  + '/tiny/batch16_lr' + str(args.base_lr)+'_testall')
