...
def parse_option():
    ...
    parser.add_argument('--output', default='/disk3/wjr/workspace/sec_proj4/baseline_expeript/global_feiqu2cls/224/contra_20251121/', type=str, metavar='PATH',
...
def build_phe_loader(args):
    ...
    # dataset_train = Shanxi_Dataset(args.data_root + 'seg_rec_mask_img_1024',
    ...
    #                                data_transform=transform)
    dataset_train = Shanxi_wmask_feiqu2_Dataset(args.data_root + 'seg_rec_img_1024', args.data_root + '/seg_rec_mask_1024', args.data_root + '/subregionsimgs', args.data_root + '/subregionsmasks',
                                   txtpath=args.data_root + 'stage1_health_txt/selected_dcm_saomiao_900_health308_sick_592/train_ilo_sub.txt',
                                   data_transform=global_transfo2,
                                   )
...
def main(config, ptname='model_ema_best_auc_shanxi_val'):
    ...
    if os.path.isfile(ckp_path):
        ...
        return shanxi_train_auc, shanxi_train_accuracy, shanxi_train_sensitivity, shanxi_train_specificity, shanxi_val_auc, shanxi_val_accuracy, shanxi_val_sensitivity, shanxi_val_specificity, shanxi_test_auc, shanxi_test_accuracy, shanxi_test_sensitivity, shanxi_test_specificity, guizhou_test_auc, guizhou_test_accuracy, guizhou_test_sensitivity, guizhou_test_specificity
    ...
    for epoch in range(config.TRAIN.START_EPOCH, config.TRAIN.EPOCHS):
        ...
        loss, auc, acc, writer = validate(data_loader_train2, model,epoch, writer, index='shanxi_val')
        # loss, auc_ema, acc_ema, writer = validate(data_loader_val, model_ema.ema, epoch, writer, index='shanxi_val_ema')
        ...
        test_loss, test_auc,test_acc, writer = validate(data_loader_test, model, epoch, writer, index='shanxi_test')
        # test_loss, test_auc_ema, test_acc_ema, writer = validate(data_loader_test, model_ema.ema, epoch, writer, index='shanxi_test_ema')
        loss_test2, auc_test2, test_acc2,writer = validate(data_loader_test2, model, epoch, writer, index='guizhou_test')
        ...
        # if test_acc_ema > max_auc_ema_avg3:
        #     best_epoch = epoch
        #     max_auc_ema_avg3 = max(max_auc_ema_avg3, test_acc_ema)
        #     save_test_checkpoint(config, epoch, model_ema.ema, max_auc_ema_avg3, optimizer, lr_scheduler,
        #                          loss_scaler,
        #                          logger, ptname='model_ema_best_acc_shanxi_test.pth')
        # else:
        #     max_auc_ema_avg3 = max(max_auc_ema_avg3, test_acc_ema)
        #
...
def train_one_epoch(config, model, criterion, data_loader, optimizer, epoch, mixup_fn, lr_scheduler, loss_scaler, model_ema=None, writer=None):
    ...
    for idx, (images, masks, labels, imgnames, is_sub) in enumerate(data_loader):
        ...
        cls_fea, patch_fea, main_output0, sub_output= model(samples, is_training=True)

        main_output=main_output0[is_sub==False]
        sub_output=main_output0[is_sub==True]
        main_targets = targets[is_sub == False]
        sub_targets = targets[is_sub == True]
        if main_targets.shape[0]>0:
            loss_cls_main = criterion(main_output, main_targets)
        else:
            loss_cls_main=torch.tensor(0.).to(device)
        if sub_targets.shape[0] > 0:
            loss_cls_sub = criterion(sub_output, sub_targets)
        else:
            loss_cls_sub=torch.tensor(0.).to(device)
        ...
        # cam_loss, loss_diff1, loss_contraii = criterion2(labels, global_cam_sick, masks,
        #                                                  masks_index=2)

        loss = loss_cls_main+loss_cls_sub
        optimizer.zero_grad()
        ...
        loss_cls_meter.update(loss_cls_main.item(), main_targets.size(0))
        loss_sub_cls_meter.update(loss_cls_sub.item(), sub_targets.size(0))
        ...
        if idx % config.PRINT_FREQ == 0:
            ...
            logger.info(
                ...
                f'loss_main_cls {loss_cls_meter.val:.4f} ({loss_cls_meter.avg:.4f})\t'
            ...
            writer.add_scalar('Shanxitrain main cls loss', loss_cls_main.item(), epoch * num_steps + idx)
            writer.add_scalar('Shanxitrain sub cls loss', loss_cls_sub.item(), epoch * num_steps + idx)
        ...
        for qq in range(len(aug_gaussian)):
            for pp in range(len(aug_rot)):
                for jj in range(len(model_name)):
                    ...
                    config.OUTPUT = os.path.join(args.output, 'wmask_nosubcls_nodropout',args.model_name,
                    ...
                    for pt_name in pt_names:
                        shanxi_train_auc, shanxi_train_acc, shanxi_train_sens, shanxi_train_spec, shanxi_val_auc, shanxi_val_acc, shanxi_val_sens, shanxi_val_spec, shanxi_test_auc, shanxi_test_accuracy, shanxi_test_sensitivity, shanxi_test_specificity, guizhou_test_auc, guizhou_test_accuracy, guizhou_test_sensitivity, guizhou_test_specificity = main(
                            config, pt_name)
                        shanxi_train_avgs = (
