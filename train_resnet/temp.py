...
def build_phe_loader(args):
    ...
    transform_list = [
        transforms.RandomResizedCrop(336, scale=(0.75, 1.), interpolation=Image.BICUBIC),
...
from model_cls.pneullm.mm_adaptation import phellm, phellm_vision, phellm_vision_simple
...
def main(config, ptname='model_ema_best_auc_shanxi_val'):
    ...
    if args.model_name=='resnet50':
        ...
        model=phellm_vision()
    elif args.model_name == 'pneullm_vision_simple':
        model=phellm_vision_simple()
...
def train_one_epoch(config, model, criterion, data_loader, optimizer, epoch, mixup_fn, lr_scheduler, loss_scaler, model_ema=None, writer=None):
    ...
    for idx, (images, masks, labels,_) in enumerate(data_loader):
...
if __name__ == '__main__':
    ...
    # lrs = [5e-5]
    lrs = [1e-4,3e-4, 5e-4, 8e-4]
    batch_sizes = [16]
    # model_name = ['hrnet18','resnet50', 'vit-base','swintiny']
    model_name = ['pneullm_vision_simple',]
    aug_rot=[True]
    aug_gaussian=[True]
    loss_type=['ce']
            ...
            for pp in range(len(aug_rot)):
                for jj in range(len(model_name)):
                    ...
                    config.OUTPUT = os.path.join(args.output, 'wmask_nosubcls_nodropout',args.model_name,
                                                 '336_batch16_lr' + str(args.base_lr)+'_testall')
