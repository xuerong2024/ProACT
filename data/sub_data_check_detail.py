...
txt_path='/disk3/wjr/dataset/nejm/shanxidataset/chinese_ilo_book_subregions_pq.txt'
# 读取文件并提取冒号后的部分
...
with open(txt_path, 'r', encoding='utf-8') as f:
    for line in f:
        ...
        region_part_clean = parts[-1].split(':', 1)[0]
        ...
        observed_region.add(region_part_clean)

        # 存储 combo
        combinations[region_part_clean][yinying_type].append(combo)
...
for region in sorted_region:
    print(f"\n--- region: {region} ---")
    for yinying_type in sorted_yinying:

        combo_list = combinations[region][yinying_type]
        if combo_list:
            counts = Counter(combo_list)
            for combo, count in counts.most_common():
                print(f" {region}: {yinying_type}:  {combo}: {count}")
        else:
