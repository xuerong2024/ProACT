...
class UniRepLKNet(nn.Module):
    ...
    def init_weights(self):
        ...
        if ckpt_path is None:
            ...
            msg = self.load_state_dict(_state_dict, strict=False)
            print(msg)
...
class LayerNorm(nn.Module):
    ...
    def forward(self, x):
        if self.data_format == "channels_last":
            ...
            return x
def load_with_key(model, cache_file):
    checkpoint = torch.load(cache_file, map_location='cpu')
    # checkpoint = checkpoint['model']
    model.load_state_dict(checkpoint)
def initialize_with_pretrained(model, model_name, cache_file):
    load_with_key(model, cache_file)
...
def unireplknet_s(**kwargs):
    model = UniRepLKNet(depths=UniRepLKNet_S_B_L_XL_depths, dims=(96, 192, 384, 768), **kwargs)
    initialize_with_pretrained(model, 'unireplknet_s', '/disk3/wjr/workspace/sec_nejm/unireplknet_s_in22k_to_in1k_384_acc86.44.pth')
    return model
