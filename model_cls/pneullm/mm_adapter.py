...
from model_cls import pneullm
from model_cls.pneullm import eval_model
...
def set_rpo_MMAdapter(model, method, dim=8, s=1, set_forward=True,t=10,gradient_checkpointing=False):
    if method == 'block':
        # not support right now
        assert NotImplementedError
        for _ in model.children():
            if type(_) ==  pneullm.rpo_model.TransformerBlock or type(_) == eval_model.TransformerBlock:
                ...
                set_rpo_MMAdapter(_, method, dim, s,set_forward=set_forward,t=t,gradient_checkpointing=gradient_checkpointing)
...
def set_MMAdapter(model, method, dim=8, s=1, set_forward=True,t=10,gradient_checkpointing=False):
    if method == 'block':
        ...
        assert NotImplementedError
        for _ in model.children():
            if type(_) ==  pneullm.model.TransformerBlock or type(_) == eval_model.TransformerBlock:
                _.adapter_attn = RepAdapter_Router(_.dim,hidden_dim=dim,scale=s,t=t)
                _.adapter_mlp = RepAdapter_Router(_.dim,hidden_dim=dim,scale=s,t=t)
                _.s = s
                _.t = t
        ...
        for _ in model.children():
            ...
            if type(_) == pneullm.model.TransformerBlock or type(_) == pneullm.eval_model.TransformerBlock:
                _.adapter_attn = RepAdapter_Router(_.dim,hidden_dim=dim,scale=s,t=t)
                # _.adapter_attn = RepAdapter(_.dim, hidden_dim=dim, scale=s) #FY0829
                _.s = s
                _.t=t
                _.gradient_checkpointing = gradient_checkpointing
                if type(_) == pneullm.eval_model.TransformerBlock:
                    bound_method = forward_llama_attn_cache.__get__(_, _.__class__)
                else:
                    bound_method = forward_llama_attn.__get__(_, _.__class__)
                if set_forward:
...
from model_cls.pneullm.clip.model import ResidualAttentionBlock
...
def set_Clip_Conv_Adapter(model, method, dim=8, s=1, set_forward=True):
    for _ in model.children():
        if type(_) == ResidualAttentionBlock:
            if method=='router':
                _.adapter_attn = RepconvAdapter(768, hidden_dim=dim, scale=s)
            elif method=='router_block':
                _.adapter_attn = RepconvAdapter(768, hidden_dim=dim, scale=s)
                _.adapter_mlp = RepconvAdapter(768, hidden_dim=dim, scale=s)
            else:
                _.adapter_attn = RepconvAdapter(768, hidden_dim=dim, scale=s)
            _.s = s
            if method=='router_block':
            ...
            set_Clip_Conv_Adapter(_, method, dim, s, set_forward=set_forward)