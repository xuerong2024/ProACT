...
from model_cls.pneullm import clip
...
class Transformer(nn.Module):
    def __init__(self, params: ModelArgs):
        ...
        self.adapter_output_cls = Linear(params.dim, 2, bias=True)

        self.freqs_cis = precompute_freqs_cis(
            self.params.dim // self.params.n_heads, self.params.max_seq_len * 2
        )
        # self.backbone = conv_backbone().float()
        self.backbone = clip.load('ViT-L/14@336px')[0]
    ...
    def define_mask(self):
        ...
        # visual_mask[:att_size-self.params.RPO_K, -1 * self.params.RPO_K:] = float("-inf")
        visual_mask[:, -1 * self.params.RPO_K:] = float("-inf")
        # visual_mask[-1 * self.params.RPO_K:, -1 * self.params.RPO_K:] = float("-inf")
        #####
    ...
    def forward(self, images):
        # images_embeds = self.backbone(images).half()  # [B, 3, 224, 224] --> [B, 6, 1024]
        images_embeds,patch_embeds = self.backbone.encode_image(images,multiscale=self.params.multiscale)  # [B, 3, 224, 224] --> [B, 6, 1024]
        # img_prompt = self.img_prompt.repeat(images_embeds.shape[0], 1, 1)
        # img_prompt= self.img_prompt.repeat(images_embeds.shape[0], 1, 1)\
        #             +self.prompt_meta_net(images_embeds.permute(0,2,1).float()).permute(0,2,1)
        ...
        # images_embeds = torch.cat([images_embeds, self.img_prompt.repeat(images_embeds.shape[0], 1, 1)], dim=1)
        images_embeds = self.adapter_proj(images_embeds)  # [B, 6, 1024] --> [B, 6, 4096]
        # images_embeds = self.adapter_proj(images_embeds).unsqueeze(1)  # [B, 6, 1024] --> [B, 6, 4096]
        h = images_embeds
...
class Transformer_vision(nn.Module):
    def __init__(self):
        ...
        # self.output = Linear(params.dim, params.vocab_size, bias=False)
        # self.adapter_output_cls = Linear(params.dim, 2, bias=False)
        self.adapter_output_cls = Linear(4096, 2, bias=True)
        # self.backbone = conv_backbone().float()
        self.backbone = clip.load('ViT-L/14@336px')[0].visual
        self.norm = RMSNorm(4096, eps=1e-6)
        # self.backbone = clip.load('ViT-B/32')[0]
        # self.backbone = clip.load('RN50')[0]
        # initialization token
        self.adapter_proj = AdapterMLP(1024, 128, 4096).float()
        self.prompt_meta_net = nn.Sequential(OrderedDict([
            ("linear1", nn.Linear(1024, 12)),
            ("relu", nn.ReLU(inplace=True)),
            ("linear2", nn.Linear(12, 4))
        ]))
        self.dtype = self.backbone.conv1.weight.dtype

    def forward(self, images):
        # images_embeds = self.backbone(images).half()  # [B, 3, 224, 224] --> [B, 6, 1024]
        images_embeds,patch_embeds = self.backbone(images.type(self.dtype),multiscale=4)  # [B, 3, 224, 224] --> [B, 6, 1024]
        img_prompt = self.prompt_meta_net(images_embeds.float()).permute(0,2,1)
        img_prompt = F.softmax(img_prompt, dim=-1)@images_embeds.float()
        images_embeds = torch.cat([images_embeds, img_prompt], dim=1)
        # images_embeds = torch.cat([images_embeds, self.img_prompt.repeat(images_embeds.shape[0], 1, 1)], dim=1)
        images_embeds = self.adapter_proj(images_embeds)  # [B, 6, 1024] --> [B, 6, 4096]
        h = self.norm(images_embeds)  # [B, 6, 4096]
        feature = torch.mean(h, dim=1)  # [B, 4096]
        pred = self.adapter_output_cls(feature.float()) # [B, 2]
        return pred
class Transformer_vision_simple(nn.Module):
    def __init__(self):
        super().__init__()
        # NOTICE: do not set ignore_index
        self.criterion = torch.nn.CrossEntropyLoss()
        ...
        # self.backbone = conv_backbone().float()
        self.backbone = clip.load('ViT-L/14@336px')[0].visual
        self.norm = RMSNorm(1024, eps=1e-6)

        self.dtype = self.backbone.conv1.weight.dtype
        self.adapter_output_cls = Linear(1024, 2, bias=True)
    ...
    def forward(self, images):
        ...
        pred = self.adapter_output_cls(feature.float()) # [B, 2]
        return pred
class Transformer_dino(nn.Module):
    def __init__(self, params: ModelArgs):
        super().__init__()
        self.params = params
        self.vocab_size = params.vocab_size
    ...
    def forward(self, images, labels, return_preds=False):
        # images_embeds = self.backbone(images).half()  # [B, 3, 224, 224] --> [B, 6, 1024]
        images_embeds,patch_embeds = self.backbone.encode_image(images,multiscale=self.params.multiscale)  # [B, 3, 224, 224] --> [B, 6, 1024]
        # img_prompt = self.img_prompt.repeat(images_embeds.shape[0], 1, 1)
        # img_prompt= self.img_prompt.repeat(images_embeds.shape[0], 1, 1)\
        ...
        h = self.norm(h)  # [B, 6, 4096]
        feature = torch.mean(h, dim=1).float()  # [B, 4096]
        pred = self.adapter_output_cls(feature) # [B, 2]
