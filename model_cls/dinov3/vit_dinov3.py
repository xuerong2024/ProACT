...
from model_cls.dinov3.layers import LayerScale, Mlp, PatchEmbed, RMSNorm, RopePositionEmbedding, SelfAttentionBlock, SwiGLUFFN
from model_cls.dinov3.utils import named_apply
import torch
...
class DinoVisionTransformer(nn.Module):
    def __init__(
        ...
        **ignored_kwargs,
        ...
        if main_class!=0:
            self.main_classifier = DINOHead(in_dim=embed_dim, out_dim=main_class)
            self.main_classifier = DINOHead(in_dim=embed_dim, out_dim=main_class)
        else:
            self.main_classifier = nn.Identity()
        if sub_class!=0:
            self.sub_classifier = DINOHead(in_dim=embed_dim, out_dim=sub_class)
            self.sub_classifier = DINOHead(in_dim=embed_dim, out_dim=sub_class)
        else:
            self.sub_classifier = nn.Identity()
    ...
    def forward(self, *args, is_training: bool = False, **kwargs):
        ...
        return self.main_classifier(ret["x_norm_clstoken"])
        # return self.main_classifier(ret["x_norm_clstoken"]), ret["x_norm_clstoken"]
        # if is_training:
        #     # return ret["x_norm_clstoken"], ret["x_norm_patchtokens"], self.main_classifier(ret["x_norm_clstoken"]), self.sub_classifier(ret["x_norm_clstoken"])
        #     return self.main_classifier(ret["x_norm_clstoken"]), ret["x_norm_clstoken"]
        #
        #     # return ret["x_norm_clstoken"], ret["x_norm_patchtokens"], self.head(ret["x_norm_clstoken"])
        #     # return ret["x_norm_clstoken"], self.head(ret["x_norm_clstoken"])
        # else:
        #     return self.main_classifier(ret["x_norm_clstoken"])
class DinoVisionTransformer_Pooling(nn.Module):
    def __init__(
        self,
        *,
        img_size: int = 224,
    ...
    def forward(self, *args, is_training: bool = False, **kwargs):
        ...
        # else:
        #     return self.main_classifier(ret["x_norm_clstoken"])
class DinoVisionTransformerTree(nn.Module):
    def __init__(
        self,
        *,
        img_size: int = 224,
        ...
        **ignored_kwargs,
        ...
        self.mask_token = nn.Parameter(torch.empty(1, embed_dim, device=device))
        self.detailed_sick_fc = nn.Linear(embed_dim, 3)
        self.detailed_health_fc = nn.Linear(embed_dim, 2)
    ...
    def forward(self, *args, is_training: bool = False, **kwargs):
        ret = self.forward_features(*args, **kwargs)
        _clsout = self.head(ret["x_norm_clstoken"])
        weights = self.head.weight
        fea_sick = ret["x_norm_clstoken"] * weights[0].unsqueeze(0)
        fea_health = ret["x_norm_clstoken"] * weights[1].unsqueeze(0)
        _detailed_sick_clsout = self.detailed_sick_fc(fea_sick)
        _detailed_health_clsout = self.detailed_health_fc(fea_health)
        patch_fea=ret["x_norm_patchtokens"].permute(0,2,1)
        hw=patch_fea.shape[-1]
        h=int(hw ** 0.5)
        patch_fea=patch_fea.view(-1, patch_fea.shape[1], h,h)

        return _clsout, _detailed_sick_clsout, _detailed_health_clsout, patch_fea, ret["x_norm_clstoken"]
        # if is_training:
        #     return ret
        # else:
...
def vit_small_tree(patch_size=16, **kwargs):
    model = DinoVisionTransformerTree(
        patch_size=patch_size,
        embed_dim=384,
        depth=12,
        num_heads=6,
    ...
    return model
def vit_base(patch_size=16, sub_class=0, main_class=0,**kwargs):
    model = DinoVisionTransformer(
        patch_size=patch_size,
        embed_dim=768,
        depth=12,
...
def vit_base_tree(patch_size=16, **kwargs):
    model =DinoVisionTransformerTree(
        patch_size=patch_size,
        embed_dim=768,
        depth=12,
