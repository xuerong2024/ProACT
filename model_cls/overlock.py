...
class DynamicConvBlock(nn.Module):
    ...
    def _forward_inner(self, x, h_x, h_r):
        ...
        x1 = na2d_av(attn1, value[0], kernel_size=self.smk_size, dilation=1)
        x2 = na2d_av(attn2, value[1], kernel_size=self.kernel_size, dilation=1)
...
def overlock_xt(pretrained=False, pretrained_cfg=None, **kwargs):
    ...
    if pretrained:
        pretrained = '/disk1/wjr/.cache/torch/hub/checkpoints/overlock_xt_in1k_224.pth'
...
def overlock_t(pretrained=True, pretrained_cfg=None, **kwargs):
    ...
    if pretrained:
        pretrained = '/disk1/wjr/.cache/torch/hub/checkpoints/overlock_t_in1k_224.pth'
        state_dict = torch.load(pretrained)
        msg = model.load_state_dict(state_dict, strict=False)
        print(msg)
...
def overlock_s(pretrained=False, pretrained_cfg=None, **kwargs):
    ...
    if pretrained:
        pretrained = '/disk1/wjr/.cache/torch/hub/checkpoints/overlock_s_in1k_224.pth'
        state_dict=torch.load(pretrained)
...
if __name__ == '__main__':
    model=overlock_xt(pretrained=True)
    model.head[-1] = nn.Conv2d(1024, 2, kernel_size=1)
    aa=torch.rand((2,3,224,224))
    bb=model(aa)
    print(bb.shape)