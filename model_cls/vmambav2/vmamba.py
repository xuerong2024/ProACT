    ...
    def __initv2__(
        ...
        FORWARD_TYPES = dict(
            ...
            v05=partial(self.forward_corev2, force_fp32=False, SelectiveScan=SelectiveScanOflex, no_einsum=True,
                        CrossScanFunc=CrossScanFunc, CrossMergeFunc=CrossMergeFunc),
            # ===============================
    ...
    def _forward(self, input: torch.Tensor, return_paras=False):
        ...
        if self.ssm_branch:
            if not return_paras:
                ...
                if self.post_norm:
                    x1, dts, Bs, Cs = self.op(x, return_paras=return_paras)
                    x = x + self.drop_path(self.norm(x1))
                else:
                    x1, dts, Bs, Cs = self.op(self.norm(x), return_paras=return_paras)
...
class VSSM(nn.Module):
    ...
    def forward(self, x, return_maps=False, return_paras=False, last_fea_return=False):
        if return_paras:
            dtss=[]
            Bss=[]
            Css=[]
        if not return_maps:
                ...
                if return_paras:
                    for block in layer.blocks:
                        ...
                        if block.Crosstype == 'org':
                            ...
                            H=int(math.sqrt(HW))
                            G=C//4
                            dts = dts.view(B, G, -1, H, H)
                            dts = dts[:, 0, ...].mean(dim=1)
                            dtss.append(dts)
                            Bss.append(Bs)
                            Css.append(Cs)
            ...
            for layer in self.layers:
                for block in layer.blocks:
                    x, dts, Bs, Cs = block(x, return_paras=True)
                    if block.Crosstype == 'org':
                        maps.append(x.mean(dim=-1))
                        if return_paras:
                            B, C, HW = dts.shape
...
class DINO_VSSM(nn.Module):
    ...
    def forward(self, x, distill_token=False, last_attn_return=False, last_fea_return=False, return_paras=False):
        x = self.patch_embed(x)
        for layer in self.layers:
            x = layer(x)
        norm_x = self.classifier.norm(x).permute(0, 3, 1, 2)
        avgpool_x = self.classifier.avgpool(norm_x)
        avgpool_x = self.classifier.flatten(avgpool_x)
        x = self.classifier.head(avgpool_x)

        return x, avgpool_x
        # return x, avgpool_x,norm_x
        ...
        #     x = self.classifier.head(avgpool_x)
        #     if distill_token:
        #         return x, norm_x.flatten(start_dim=2, end_dim=3).permute(0,2,1), avgpool_x
        #     else:
        #         return x
        # else:
        #     if return_paras:
        ...
        #                     H = int(math.sqrt(HW))
        #                     G = C // 4
        #                     dts = dts.view(B, -1, G, HW)
        #                     dts = dts[:, 0:2] + dts[:, 2:4].flip(dims=[-1]).view(B, 2, G, -1)
        #                     dts = dts[:, 0] + dts[:, 1].view(B, -1, H, H).transpose(dim0=2, dim1=3).contiguous().view(B, G,-1)
        #                     dts = dts.mean(dim=1).view(B, -1, H, H).squeeze(1)
        #                     Bs = Bs.view(B, -1, H, H)
        #                     Bs = Bs[:, 0, ...]
        #                     Cs = Cs.view(B, -1, H, H)
        #                     Cs = Cs[:, 0, ...]
        ...
        #         if last_attn_return:
        #             if return_paras:
        #                 return out, maps, last_fea, dtss
        #             else:
        #                 return out, maps, last_fea
        #         else:
        #             if return_paras:
        #                 return out, last_fea, dtss
        #             else:
        #                 return out, last_fea
        #     else:
        #         x = self.classifier(x)
...
if __name__ == "__main__":
    ...
    model = DINO_VSSM().cuda()
    input = torch.ones((2, 3, 512, 512)).cuda()
    y = model(input, return_paras=False)
    print(y.shape)