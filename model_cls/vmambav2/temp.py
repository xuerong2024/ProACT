...
try:
    ...
    from .csms6s import CrossScan, CrossMerge, Cross_Recombination_Scan, Cross_Recombination_Merge
...
class SS2Dv2:
    def __initv2__(
            ...
            channel_first=False,
            # CrossScan=CrossScan,
            CrossScanFunc=Cross_Recombination_Scan,
            # CrossMerge=CrossMerge,
            CrossMergeFunc=Cross_Recombination_Merge,
        ...
        FORWARD_TYPES = dict(
            ...
            v052dc=partial(self.forward_corev2, force_fp32=False, SelectiveScan=SelectiveScanOflex, no_einsum=True,
                           cascade2d=True),
            # ===============================
            v2=partial(self.forward_corev2, force_fp32=(not self.disable_force32), SelectiveScan=SelectiveScanCore, CrossScanFunc=CrossScanFunc, CrossMergeFunc=CrossMergeFunc),
            v3=partial(self.forward_corev2, force_fp32=False, SelectiveScan=SelectiveScanOflex, CrossScanFunc=CrossScanFunc, CrossMergeFunc=CrossMergeFunc),
    ...
    def forward_corev2(
        ...
        if cascade2d:
            ...
            y: torch.Tensor = CrossMergeFunc.apply(ys)
        ...
        y = out_norm(y)
        if return_paras:
            dts = torch.nn.functional.softplus(dts + delta_bias[:, None])
            return (y.to(x.dtype) if to_dtype else y), dts, Bs, Cs
        else:
            return (y.to(x.dtype) if to_dtype else y)
        # return (y.to(x.dtype) if to_dtype else y)

    def forwardv2(self, x: torch.Tensor, return_paras=False, **kwargs):
        x = self.in_proj(x)
        ...
        x = self.act(x)
        if return_paras:
            y, dts, Bs, Cs = self.forward_core(x, return_paras=return_paras)
        else:
            y = self.forward_core(x, return_paras=return_paras)
        y = self.out_act(y)
        if not self.disable_z:
            y = y * z
        out = self.dropout(self.out_proj(y))
        if return_paras:
            return out, dts, Bs, Cs
        else:
            return out
...
class SS2D(nn.Module, mamba_init, SS2Dv0, SS2Dv2, SS2Dv3):
    def __init__(
        ...
        kwargs.update(
            ...
            initialize=initialize, forward_type=forward_type, channel_first=channel_first, CrossScanFunc=CrossScanFunc, CrossMergeFunc=CrossMergeFunc,
        )
...
class VSSBlock(nn.Module):
    def __init__(
        ...
        self.CrossScanFunc=CrossScanFunc
        if CrossScanFunc==CrossScan:
            self.Crosstype='org'
        else:
        ...
        if self.ssm_branch:
            ...
            self.op = SS2D(
                ...
                CrossScanFunc=CrossScanFunc,
                CrossMergeFunc=CrossMergeFunc,
            )
    ...
    def _forward(self, input: torch.Tensor, return_paras=False):
        x = input
        if self.ssm_branch:
            if not return_paras:
                if self.post_norm:
                ...
                if self.post_norm:
                    x, dts, Bs, Cs = self.op(x, return_paras=return_paras)
                    x = x + self.drop_path(self.norm(x))
                else:
                    x, dts, Bs, Cs = self.op(self.norm(x), return_paras=return_paras)
    ...
    def forward(self, input: torch.Tensor, return_paras=False):
        if self.use_checkpoint:
            return checkpoint.checkpoint(self._forward, input)
        else:
            return self._forward(input, return_paras=return_paras)


class PatchUnMerging2D(nn.Module):
    ...
    def __init__(self, dim, norm_layer=nn.LayerNorm):
        super().__init__()
        self.dim = dim
        self.reduction = nn.ConvTranspose2d(dim * 2, dim, kernel_size=2, stride=2)
        # self.reduction = nn.Linear(dim*2, dim, bias=False)
        self.norm = norm_layer(dim)
        # self.recovery=nn.PixelShuffle(2)
    ...
    def forward(self, x):
        ...
        # new_x[:, 0::2, 0::2, :]=x1
        # new_x[:, 1::2, 0::2, :]=x2
        # new_x[:, 0::2, 1::2, :]=x3  # B H/2 W/2 C
        # new_x[:, 1::2, 1::2, :]=x4  # B H/2 W/2 C
        x = self.norm(x)
        return x
...
class VSSM(nn.Module):
    def __init__(
            ...
            return_maps=False,
            # CrossScan=CrossScan,
            CrossScan=Cross_Recombination_Scan,
            # CrossMerge=CrossMerge,
            CrossMerge=Cross_Recombination_Merge,
            **kwargs,
    ):
        super().__init__()
        ...
        for i_layer in range(self.num_layers):
            ...
            self.layers.append(self._make_layer(
                ...
                # CrossMerge=CrossMerge,
                # # CrossMerge=Cross_Recombination_Merge,
                change=True if i_layer<2 else False,
            ))
            # if return_maps:
            ...
            #     else:
            #         dimm = dims[i_layer - 1]
            #     upsample = PatchUnMerging2D(dim=dimm, norm_layer=norm_layer)
    ...
    def _make_layer(
        ...
        for d in range(depth):
            blocks.append(VSSBlock(
                ...
                gmlp=gmlp,
                use_checkpoint=use_checkpoint,
                CrossScanFunc=CrossScan if (d%2!=0) or (change==False) else Cross_Recombination_Scan,
                # CrossScanFunc=CrossScan if (change == False) else Cross_Recombination_Scan,
                # CrossScanFunc=CrossScan if d % 2 == 0 else Cross_Recombination_Scan,
                # CrossScanFunc=CrossScan,
                CrossMergeFunc=CrossMerge if (d%2!=0) or (change==False) else Cross_Recombination_Merge,
                # CrossMergeFunc=CrossMerge if (change == False) else Cross_Recombination_Merge,
                # CrossMergeFunc=CrossMerge if d % 2 == 0 else Cross_Recombination_Merge,
                # CrossMergeFunc=CrossMerge,
            ))
    ...
    def forward(self, x, return_maps=False, return_paras=False):
        if return_paras:
            dtss=[]
            Bss=[]
            Css=[]
        if not return_maps:
            ...
            for layer in self.layers:
                if return_paras:
                    for block in layer.blocks:
                        x, dts, Bs, Cs = block(x, return_paras=return_paras)
                        if block.Crosstype == 'org':
                            dtss.append(dts)
                            Bss.append(Bs)
                            Css.append(Cs)
            ...
            x = self.classifier(x)
            if return_paras:
                return x, dtss, Bss, Css
            else:
                return x
        else:
            maps = []
            x = self.patch_embed(x)
            # maps.append(x.mean(dim=-1))
            # for layer, upsample in zip(self.layers, self.upsamples):
            #     x = layer(x)
            #     map = upsample(x)
            #
            #     maps.append(map.mean(dim=-1))
            for layer in self.layers:
                if return_paras:
                    for block in layer.blocks:
                        x, dts, Bs, Cs = block(x, return_paras=return_paras)
                    ...
                    map=layer.blocks(x)
                    x=layer.downsample(map)
                    maps.append(map.mean(dim=-1))
            # maps.append(x.mean(dim=-1))
            x = self.classifier(x)
            if return_paras:
                return x, maps, dtss, Bss, Css
            else:
                return x, maps
...
class DINO_VSSM(nn.Module):
    def __init__(
        ...
        if isinstance(dims, int):
            dims = [int(dims * 2 ** i_layer) for i_layer in range(self.num_layers)]
        self.num_features = self.embed_dim = dims[-1]
    ...
    def forward(self, x, last_attn_return=False, last_fea_return=False):
        if not last_attn_return and not last_fea_return:
            x = self.patch_embed(x)
            for layer in self.layers:
                x = layer(x)
            x = self.classifier(x)
            ...
            if last_fea_return:
                last_fea = self.classifier.flatten(
                    self.classifier.avgpool(self.classifier.permute(self.classifier.norm(x))))
                out = self.classifier.head(last_fea)
                if last_attn_return:
                    return out, maps, last_fea
                else:
                    return out, last_fea
            else:
                x = self.classifier(x)
...
class DINO_VSSM_ENHANCE(nn.Module):
    def __init__(
            self,
            patch_size=4,
            in_chans=3,
        ...
        if self.return_maps:
            self.upsamples = nn.ModuleList()
        self.layers = nn.ModuleList()
        self.spa_enhancenet = nn.ModuleList()
        self.gammas = []
        for i_layer in range(self.num_layers):
            downsample = _make_downsample(
                self.dims[i_layer],
            ...
            self.spa_enhancenet.append(
                nn.Conv2d(self.dims[i_layer], self.dims[i_layer], kernel_size=3, stride=1, padding=1,
                          groups=self.dims[i_layer]))

            self.gammas.append(nn.Parameter(torch.zeros((1, self.dims[i_layer], 1, 1)), requires_grad=True))
            if return_maps:
                if (i_layer < self.num_layers - 1):
                    dimm = dims[i_layer]
                else:
                ...
                upsample = nn.Identity()
                self.upsamples.append(upsample)
        self.gammas = nn.ParameterList(self.gammas)  # 转换为 nn.ParameterList
    ...
    def forward(self, x, last_attn_return=False, last_fea_return=False):
        if not last_attn_return and not last_fea_return:
            x = self.patch_embed(x)
            for layer, ehnlayer, gamma in zip(self.layers, self.spa_enhancenet, self.gammas):
                x_org = x.clone()
                x_org = x_org.permute(0, 3, 1, 2)
                x = layer.blocks(x)
                x=x.permute(0, 3, 1, 2)
                map = torch.sigmoid(ehnlayer(x_org))
                x=(x+gamma*x_org*(map)).permute(0, 2,3,1)
                x = layer.downsample(x)
            x = self.classifier(x)
            return x
        else:
...
class SDD_VSSM(nn.Module):
    ...
    def forward(self, x, return_maps=False, patch_score_return=False):
        if not return_maps:
            x = self.patch_embed(x)
            for layer in self.layers:
                x = layer(x)
            if patch_score_return:
                ...
                x = self.classifier.permute(x)
                x_spp = self.spp(x)
                x_spp = x_spp.permute((2, 0, 1))
                m, b, c = x_spp.shape[0], x_spp.shape[1], x_spp.shape[2]
                x_spp = torch.reshape(x_spp, (m * b, c))
                patch_score = self.classifier.head(x_spp)
                patch_score = torch.reshape(patch_score, (m, b, patch_score.shape[-1]))
                patch_score = patch_score.permute((1, 2, 0))
                output_score = self.classifier.avgpool(x)
                output_score = self.classifier.flatten(output_score)
                output_score = self.classifier.head(output_score)
                return output_score, patch_score
            else:
                x = self.classifier(x)
                return x
        else:
...
if __name__ == "__main__":
    model = VSSM(depths=[2, 2, 6, 2], return_maps=True).cuda()
    input = torch.randn((2, 3, 224, 224)).cuda()
    y = model(input, return_paras=True)
    print(y.shape)