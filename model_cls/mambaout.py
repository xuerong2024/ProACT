...
class MambaOut(nn.Module):
    ...
    def forward_features(self, x, return_maps=False):
        ...
        for i in range(self.num_stage):
            ...
            x = self.stages[i](x)
            if return_maps:
                maps.append(x.permute(0,3,1,2).mean(dim=1))
        if return_maps:
            return self.norm(x.mean([1, 2])), maps  # (B, H, W, C) -> (B, C)
        else:
            return self.norm(x.mean([1, 2])) # (B, H, W, C) -> (B, C)

    def forward(self, x, return_maps=False, last_fea_return=False):
        if last_fea_return:
            fea = self.forward_features(x, return_maps)
            x = self.head(fea)
            return x, fea
        if return_maps:
            x,maps = self.forward_features(x, return_maps)
            x = self.head(x)
            return x,maps
        else:
            x = self.forward_features(x, return_maps)
            x = self.head(x)
            return x
...
#         if_cls_token=True, if_devide_out=True, use_middle_cls_token=True).cuda()
