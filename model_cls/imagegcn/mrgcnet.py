'ImageGCN: Multi-Relational Image Graph Convolutional Networks for Disease Identification With Chest X-Rays, TMI2022'
...
from model_cls.imagegcn.layers import *

from model_cls.imagegcn.utility.preprocessing import str2value, issymmetric, to_dense, to_sparse
...
class MRGCN(nn.Module):
    ...
    """
    def __init__(self, relations, in_dim=None, out_dim=None, enc='singlevgg16bn', inchannel=3, share_encoder='partly', selfweight=1):
...
class SimpleMRGCN(nn.Module):
    """
    multi-relational GCN
    """

    def __init__(self,relations=[], in_dim=None, out_dim=None, inchannel=3, share_encoder='partly',
                 selfweight=1):
        super(SimpleMRGCN, self).__init__()
        self.selfweight = selfweight
        self.encoder = MyVggNet16_bn(inchannel=inchannel).features
    ...
    def forward(self, fea_in, k):
        fea_in = self.encoder(fea_in).squeeze()
        fea_out = fea_in[k:]
        fea = self.gcn['self'](fea_out, self.selfweight)
        fea = self.gcn['self'](fea_in, self.selfweight)
        return fea
...
class SingleLayerImageGCN(nn.Module):
    def __init__(self, relations, in_dim=1024,out_dim=14, inchannel=3, share_encoder='partly'):
        super(SingleLayerImageGCN, self).__init__()
        self.out_dim = out_dim
        self.layer = MRGCN(relations,in_dim=in_dim,out_dim=out_dim, inchannel=inchannel, share_encoder=share_encoder )
    
    def forward(self, fea,  k ):
        fea2 = self.layer(fea, k)
        fea2 = fea2.view(-1, self.out_dim)

        return fea2


class SimpleSingleLayerImageGCN(nn.Module):
    def __init__(self, relations=[], in_dim=1024, out_dim=2, inchannel=3, share_encoder='partly'):
        super(SimpleSingleLayerImageGCN, self).__init__()
        self.out_dim = out_dim
        self.layer = SimpleMRGCN(relations, in_dim=in_dim, out_dim=out_dim, inchannel=inchannel,
...
if __name__ == "__main__":
    model = SimpleSingleLayerImageGCN().cuda()
